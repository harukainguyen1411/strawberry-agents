...
- **Rationale:** Bookkeeping move per § "* → archived" rule. Plan was implemented via PR #39 but the Monitor-arming gate it introduced (PreToolUse hook with no matcher field firing on every tool call) accumulated three latent bugs over four days and was removed at commit cd20732b after consuming ~4 hours of Duong's morning. Archive is the correct terminal state. Recovery note: file was deleted off disk by a failed cleanup attempt earlier 2026-04-26; restored from blob 8f942bc1 (477 lines) and archived in a single step (skipping the implemented/ intermediate location, which is valid because the plan was already in the implemented stage in git history). Full retro: feedback/2026-04-26-convenience-promoted-to-forcing-function.md.
