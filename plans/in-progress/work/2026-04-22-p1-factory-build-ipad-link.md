...
orianna_signature_approved: "sha256:cb59eb4cf678d24d3e624e1cb9b24515ec9d33f3d7c31450e839caa34b6bd8be:2026-04-22T15:11:11Z"
orianna_signature_in_progress: "sha256:cb59eb4cf678d24d3e624e1cb9b24515ec9d33f3d7c31450e839caa34b6bd8be:2026-04-22T15:13:20Z"
---
...
Aphelios breakdown (D1A-inline, 2026-04-22). Original T.P1.1–T.P1.16 have been decomposed into sized sub-tasks with explicit TDD ordering, executor-tier annotations, and dependency chains. All tasks are 20–60 min per rule cap; T.P1.3 split into T.P1.3a + T.P1.3b per Swain's note #1. T.P1.14 simplified from canary+24h-soak to deploy+smoke per OQ-1 (no canary, internal users).

Concurrency: **S3 stream (Viktor)** and **S1 stream (Jayce)** parallelize after T.P1.0 + T.P1.11 land. They rejoin at T.P1.12 (integration) and T.P1.14 (deploy). Test-author Xayah drafts contract bodies; fault-injection fixtures are Rakan complex-track. Frontend panel is Lulu-advised Soraka impl. Deploy is Ekko. QA is Akali.

Executor-tier key: **Viktor** (complex-track Python/backend), **Jayce** (normal-track Python/backend), **Xayah** (test author), **Rakan** (complex-track test fixtures), **Soraka** (normal-track frontend), **Lulu** (frontend advisor, non-executor), **Ekko** (ops/deploy), **Akali** (Playwright QA).

### Phase A — Shared foundation (blocks both streams)

- [ ] **T.P1.0** — **Xayah** drafts the full contract-test suite bodies for T.P1.1, T.P1.1b, T.P1.7, and T.P1.12 (pytest signatures, fixture shapes, assertion lists). Writes to `company-os/tools/demo-factory/tests/test_build.py` scaffolds (marked xfail) and `company-os/tools/demo-studio-v3/tests/test_build_endpoint.py` scaffold (marked xfail). estimate_minutes: 45. Files: `company-os/tools/demo-factory/tests/test_build.py`, `company-os/tools/demo-studio-v3/tests/test_build_endpoint.py`, `company-os/tools/demo-studio-v3/tests/test_factory_bridge_v2.py`. DoD: scaffolds committed with `pytest.mark.xfail(reason="P1 not yet implemented")`, pytest collect succeeds, no import errors; references plan slug in docstring. Depends on: none. Blocks: T.P1.1, T.P1.1b, T.P1.7, T.P1.12. <!-- orianna: ok -- prospective work-workspace paths under company-os/tools/demo-factory/ and company-os/tools/demo-studio-v3/; pytest.mark.xfail is a Python identifier -->
- [ ] **T.P1.11** — **Jayce** extends `session._UPDATABLE_FIELDS` allowlist with `buildId`, `shortcode`, `projectUrl`, `demoUrl`; adds a single xfail unit test in `tests/test_session.py` asserting each new field round-trips through `update_session_field`. estimate_minutes: 15. Files: `company-os/tools/demo-studio-v3/session.py`, `company-os/tools/demo-studio-v3/tests/test_session.py`. DoD: allowlist updated at line ~177; xfail test flips to pass; existing allowlist tests still pass. Depends on: none. Blocks: T.P1.10a, T.P1.10b. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-studio-v3/; session._UPDATABLE_FIELDS and update_session_field are Python identifiers; tests/test_session.py is a prospective work-workspace file -->

### Phase B — S3 stream (Viktor) — runs in parallel with Phase C once T.P1.0 lands

- [ ] **T.P1.1** — **Xayah/Viktor** finalize xfail contract test: S3 `POST /build` returns real `buildId` + `projectId` shape under `FACTORY_REAL_BUILD=1` with S2 config present. Asserts response keys, `Authorization: Bearer` header on WS hop, no synthetic uuid pattern in response (regex-reject `proj-[a-f0-9]{8}`). estimate_minutes: 30. Files: `company-os/tools/demo-factory/tests/test_build.py`. DoD: test marked xfail, references T.P1.1; pytest collect succeeds. Depends on: T.P1.0. Blocks: T.P1.5a. <!-- orianna: ok -- prospective work-workspace test path under company-os/tools/demo-factory/tests/; proj-[a-f0-9]{8} is a regex literal -->
- [ ] **T.P1.1b** — **Xayah/Viktor** xfail unit test for `factory_client_v2.start_build` (S1-side contract mirror): verifies request URL, method POST, body keys (`sessionId`, optional `projectId`, `configVersion: 3`), `Authorization: Bearer ${FACTORY_TOKEN}` header, 30 s timeout, no retry on 5xx. estimate_minutes: 25. Files: `company-os/tools/demo-studio-v3/tests/test_factory_client_v2.py` (new). DoD: xfail marker in place; test fails for "module not found" cleanly; matches D2 contract verbatim. Depends on: T.P1.0. Blocks: T.P1.8. <!-- orianna: ok -- factory_client_v2.start_build is a Python identifier; prospective work-workspace test path under company-os/tools/demo-studio-v3/tests/ -->
- [ ] **T.P1.2** — **Viktor** implement `company-os/tools/demo-factory/config_mgmt_client.py`: sync `requests.get` on `${CONFIG_MGMT_URL}/v1/config/{session_id}`, Bearer `${CONFIG_MGMT_TOKEN}`, 10 s timeout, single-attempt, raises `NotFoundError`/`ServiceUnavailableError`/generic on failure. Copy-adapt from S1's existing `config_mgmt_client.py` shape. estimate_minutes: 25. Files: `company-os/tools/demo-factory/config_mgmt_client.py` (new), `company-os/tools/demo-factory/tests/test_config_mgmt_client.py` (new). DoD: unit test mocks httpx/requests, covers 200/404/503/timeout; module imports cleanly; no new dep added. Depends on: none. Blocks: T.P1.5a. <!-- orianna: ok -- prospective work-workspace paths under company-os/tools/demo-factory/; requests.get, NotFoundError, ServiceUnavailableError are Python identifiers; /v1/config/{session_id} is an HTTP route -->
- [ ] **T.P1.3a** — **Viktor** scaffold `company-os/tools/demo-factory/factory_build.py` module: defines `BuildFailed(Exception)` with `reason` + `detail`, `async def run_build_from_config(...)` signature per D3, `s2_config_to_factory_content(cfg)` stub raising `NotImplementedError`, required-field validation helper `_validate_config(cfg)` (raises `BuildFailed(reason="config_invalid", detail=<field list>)` when missing `brand`, `market`, `colors.primary`, `logos.wordmark`, `shortcode`). No WS calls yet. estimate_minutes: 35. Files: `company-os/tools/demo-factory/factory_build.py` (new). DoD: module imports; `BuildFailed` constructs; `_validate_config` raises with correct reason on each missing field (covered by stub xfail unit tests). Depends on: T.P1.0. Blocks: T.P1.3b, T.P1.4. <!-- orianna: ok -- prospective work-workspace path company-os/tools/demo-factory/factory_build.py; colors.primary and logos.wordmark are S2 config dict key paths; BuildFailed, NotImplementedError, run_build_from_config, s2_config_to_factory_content, _validate_config are Python identifiers -->
- [ ] **T.P1.3b** — **Viktor** wire WS-apply steps into `run_build_from_config`: steps 3–14 per §D3.1 (`create_project`, `post_clone_fixup`, `upload_logo`, `apply_ios_template`, `apply_google_template`, `apply_params`, `apply_translation_keys`, `apply_journey_actions`, `apply_token_ui`, `sync_pass_preview`, `publish_ios_template`, `get_project`). Each step emits `event_sink("step_start", …)` and `event_sink("step_complete", …)`. Step failures convert WS exceptions into `BuildFailed(reason="ws_api_failed", detail=step_name)`. Compose return `{projectId, shortcode, projectUrl, demoUrl, passUrls, configVersion}` using env-var hostnames per OQ-6. estimate_minutes: 45. Files: `company-os/tools/demo-factory/factory_build.py`. DoD: `run_build_from_config` executes end-to-end against a mocked `WSClient` (fixture), emits 12 step_start+step_complete pairs, returns D2 shape; failure at any step surfaces `ws_api_failed`. Depends on: T.P1.3a. Blocks: T.P1.5a. <!-- orianna: ok -- all backticked tokens are Python identifiers (WSClient step names, BuildFailed, event_sink, run_build_from_config) or prospective work-workspace path company-os/tools/demo-factory/factory_build.py -->
- [ ] **T.P1.4** — **Viktor** implement `s2_config_to_factory_content(cfg)` translator with full unit coverage: direct-carry fields (`brand`, `market`, `languages`, `shortcode`, `colors`, `logos`, `card`, `params`, `journey`, `tokenUi`), derived fields (`insuranceLine`, `persona`, `translations`, `demoSteps` from `ipadDemo.steps`), empty/missing-optional defaults, missing-required raises `BuildFailed(reason="config_invalid", …)`. Minimum 10 fixtures per §Test plan. estimate_minutes: 45. Files: `company-os/tools/demo-factory/factory_build.py`, `company-os/tools/demo-factory/tests/test_factory_build.py` (new). DoD: 10+ unit tests green; translator round-trips a fixture matching `tools/demo-config-mgmt/main.py::MOCK_CONFIG` into factory `content` shape; required-field omission yields `config_invalid` with field name in detail. Depends on: T.P1.3a. Blocks: T.P1.5a. <!-- orianna: ok -- prospective work-workspace paths under company-os/tools/demo-factory/; tools/demo-config-mgmt/main.py::MOCK_CONFIG is a Python symbol reference; ipadDemo.steps is a config dict key path -->
- [ ] **T.P1.6** — **Viktor** extend `build_complete` event payload in S3: add `shortcode`, `projectUrl`, `demoUrl`, `passUrls`, `configVersion` keys; keep existing keys for parity. Update mock `_run_mock_build` to emit the same shape (values synthetic) so relay continues working when flag off. estimate_minutes: 20. Files: `company-os/tools/demo-factory/main.py`. DoD: event JSON shape matches §D2 exactly (schema snapshot test); mock and real paths emit identical key sets. Depends on: T.P1.3b. Blocks: T.P1.5a, T.P1.10a. <!-- orianna: ok -- work-workspace path company-os/tools/demo-factory/main.py; _run_mock_build is a Python identifier -->
- [ ] **T.P1.5a** — **Viktor** rewrite `_run_build_job` behind `FACTORY_REAL_BUILD` env flag per §D3.3 snippet: flag unset → existing mock; flag set → `fetch_config` → `run_build_from_config` → write status + events. Error branches call `_append_build_event("build_failed", …)` with reason per §D6 taxonomy. estimate_minutes: 40. Files: `company-os/tools/demo-factory/main.py`. DoD: T.P1.1 xfail flips to pass; flag-off path preserves existing mock-test green; flag-on path writes real `projectId`+`shortcode` when given a fixture config. Depends on: T.P1.1, T.P1.2, T.P1.3b, T.P1.4, T.P1.6. Blocks: T.P1.5b, T.P1.7. <!-- orianna: ok -- work-workspace path company-os/tools/demo-factory/main.py; _run_build_job, fetch_config, run_build_from_config, _append_build_event are Python identifiers -->
- [ ] **T.P1.5b** — **Viktor** add 10-min wall-clock guard around `run_build_from_config` using `asyncio.wait_for`; emit `build_failed` with `reason="timeout"` on `asyncio.TimeoutError`; emit `build_failed` with `reason="unexpected"` on uncaught exceptions. estimate_minutes: 20. Files: `company-os/tools/demo-factory/main.py`, `company-os/tools/demo-factory/tests/test_build.py`. DoD: forced-sleep fixture trips `timeout` reason within 1 s via monkeypatched ceiling; generic-exception fixture trips `unexpected`. Depends on: T.P1.5a. Blocks: T.P1.7. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-factory/; asyncio.wait_for and asyncio.TimeoutError are Python stdlib identifiers -->
- [ ] **T.P1.7** — **Xayah drafts / Rakan fault-injects** xfail contract test: S3 emits `build_failed` with each of the five `reason` values (`config_fetch_failed`, `config_invalid`, `ws_api_failed`, `timeout`, `unexpected`) under forced failure conditions. Uses a mocked `WSClient` that raises on selectable step + a forced-timeout monkeypatch. estimate_minutes: 40. Files: `company-os/tools/demo-factory/tests/test_build.py`, `company-os/tools/demo-factory/tests/fixtures/ws_client_fault.py` (new). DoD: five test cases all green; fixture is reusable for future plans (exported). Depends on: T.P1.5b. Blocks: (none — feeds T.P1.12 context only). <!-- orianna: ok -- prospective work-workspace test paths under company-os/tools/demo-factory/tests/ -->

### Phase C — S1 stream (Jayce) — runs in parallel with Phase B once T.P1.0 + T.P1.11 land

- [ ] **T.P1.8** — **Jayce** implement `company-os/tools/demo-studio-v3/factory_client_v2.py`: `async def start_build(session_id, project_id=None) -> dict` using `httpx.AsyncClient`, `POST ${FACTORY_BASE_URL}/build` body `{sessionId, projectId?, configVersion: 3}`, `Authorization: Bearer ${FACTORY_TOKEN}` header, 30 s timeout, 1 attempt (non-idempotent per §D2). Raises typed exceptions for 4xx/5xx/network. estimate_minutes: 30. Files: `company-os/tools/demo-studio-v3/factory_client_v2.py` (new). DoD: T.P1.1b xfail flips to pass; no retry path on 5xx; returns parsed `{buildId, projectId}`. Depends on: T.P1.1b. Blocks: T.P1.9. <!-- orianna: ok -- prospective work-workspace path company-os/tools/demo-studio-v3/factory_client_v2.py; httpx.AsyncClient is a Python library identifier -->
- [ ] **T.P1.9** — **Jayce** rewrite `factory_bridge_v2.trigger_factory_v2` to call `factory_client_v2.start_build` and return `{ok, buildId, projectId}`; persist `buildId` and real `projectId` to session doc synchronously (per OQ-2 pick `a`). Deprecate `factoryRunId` write (comment-only deprecation, keep field for one release). estimate_minutes: 25. Files: `company-os/tools/demo-studio-v3/factory_bridge_v2.py`, `company-os/tools/demo-studio-v3/tests/test_factory_bridge_v2.py`. DoD: existing scaffold removed; session doc writes include `buildId` + real `projectId`; xfail in `test_factory_bridge_v2.py` (scaffolded in T.P1.0) flips to pass. Depends on: T.P1.8, T.P1.11. Blocks: T.P1.10a. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-studio-v3/; factory_bridge_v2.trigger_factory_v2 and factory_client_v2.start_build are Python dotted identifiers -->
- [ ] **T.P1.10a** — **Jayce** extend S1 SSE relay `s3_build_sse_stream` parser to decode `build_complete` event and write `shortcode`, `projectUrl`, `demoUrl`, `outputUrls={demoUrl, projectUrl, passUrls}` to session doc; transition `status=complete`. Writes `status=failed` on `build_failed`. estimate_minutes: 35. Files: `company-os/tools/demo-studio-v3/main.py` (around line 175), `company-os/tools/demo-studio-v3/tests/test_sse_relay.py` (new or extend existing). DoD: xfail unit test fixture sends a `build_complete` SSE payload → session doc mutated with all four URL fields; `build_failed` path sets `status=failed`. Depends on: T.P1.6, T.P1.9, T.P1.11. Blocks: T.P1.10b, T.P1.12. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-studio-v3/; s3_build_sse_stream is a Python identifier -->
- [ ] **T.P1.10b** — **Jayce** implement OQ-5 one-shot `GET ${FACTORY_BASE_URL}/build/{buildId}` fallback on SSE stream close before terminal event: if the SSE async iterator exits with session still in `building`, fire one sync GET and promote terminal state from response body. estimate_minutes: 30. Files: `company-os/tools/demo-studio-v3/main.py`, `company-os/tools/demo-studio-v3/tests/test_sse_relay.py`. DoD: unit test closes the SSE mid-stream after only `step_complete` events; fallback GET returns `status=success` body; session doc transitions to `complete` via fallback path. Depends on: T.P1.10a. Blocks: T.P1.12. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-studio-v3/; GET ${FACTORY_BASE_URL}/build/{buildId} is an HTTP route -->

### Phase D — Ops foundation (can start in parallel with Phase B/C)

- [ ] **T.P1.E1** — **Ekko** add env vars `WS_APP_BASE_URL`, `DEMO_BASE_URL`, `CONFIG_MGMT_URL`, `CONFIG_MGMT_TOKEN`, `FACTORY_REAL_BUILD` to S3 Cloud Run deploy config (staging + prod) per OQ-6. Default `WS_APP_BASE_URL=https://app.walletstudio.com`, `DEMO_BASE_URL=https://demo.missmp.tech`, `FACTORY_REAL_BUILD=0` pre-deploy. estimate_minutes: 25. Files: `company-os/ops/cloud-run/demo-factory/*.yaml` or equivalent deploy script. DoD: staging deploy picks up new vars (verified via `gcloud run services describe`); no code-side reads fail on startup. Depends on: none. Blocks: T.P1.14. <!-- orianna: ok -- prospective work-workspace ops path under company-os/ops/cloud-run/; gcloud run services describe is a CLI command; deployed-service URLs (already suppressed frontmatter-wide) -->
- [ ] **T.P1.E2** — **Ekko** add env vars `FACTORY_BASE_URL`, `FACTORY_TOKEN` to S1 Cloud Run deploy config (if not already present; T.P1.8 assumes them). estimate_minutes: 15. Files: S1 deploy config. DoD: staging S1 resolves `FACTORY_BASE_URL` at startup; auth header verified in staging smoke. Depends on: none. Blocks: T.P1.14.

### Phase E — Rejoin: integration + frontend + deploy + QA

- [ ] **T.P1.12** — **Rakan** xfail integration test: S1 `POST /session/{id}/build` → full S3 real pipeline (with WSClient mocked per OQ-3 pick `b` — no nightly real) → assert session doc has `status=complete`, `outputUrls.demoUrl` matches `https://demo.missmp.tech/<shortcode>`, `projectId` is numeric string, `buildId` present. Reuses Rakan's fault-injection fixture from T.P1.7. estimate_minutes: 50. Files: `company-os/tools/demo-studio-v3/tests/test_build_endpoint.py`. DoD: xfail flips to pass; full round-trip green with hermetic WS mock; <10 s wall-clock; referenced in CI `pytest` run. Depends on: T.P1.5b, T.P1.7, T.P1.10b. Blocks: T.P1.14. <!-- orianna: ok -- prospective work-workspace test path; outputUrls.demoUrl is a session doc field key path; pytest is a Python tool -->
- [ ] **T.P1.13a** — **Lulu** advises on "Demo ready" panel layout (two-line brief: heading, primary CTA button "Open iPad demo", secondary link "View in Wallet Studio", copy-icon next to URL; placement above chat log; reuse existing `studio.css` tokens). Output: 5–10 line design note appended to this plan or posted to Soraka's inbox. estimate_minutes: 20. Files: `agents/soraka/inbox/2026-04-22-demo-ready-panel-brief.md` (new). DoD: brief covers CTA copy, target attrs (`target="_blank" rel="noopener noreferrer"`), fallback when `demoUrl` missing, and keeps "Demo deployed" chat line or removes it (Lulu decides per §D5). Depends on: none. Blocks: T.P1.13b. <!-- orianna: ok -- prospective strawberry-agents inbox path agents/soraka/inbox/; studio.css is a work-workspace filename -->
- [ ] **T.P1.13b** — **Soraka** implement the panel in `company-os/tools/demo-studio-v3/static/studio.js`: replace line ~860 chat-message block with a persistent panel rendered when `d.status === 'complete'`; reads `d.outputUrls.demoUrl`, `d.outputUrls.projectUrl`; uses existing styles from `static/studio.css`. Add unit-level smoke (headless assertion via existing JS test harness if present; otherwise manual screenshot in PR). estimate_minutes: 40. Files: `company-os/tools/demo-studio-v3/static/studio.js`, optionally `company-os/tools/demo-studio-v3/static/studio.css`. DoD: panel renders on complete state; CTA has correct `target`/`rel`; click opens demo in new tab (manual verify); Lulu's brief satisfied. Depends on: T.P1.13a, T.P1.10a. Blocks: T.P1.16. <!-- orianna: ok -- work-workspace paths under company-os/tools/demo-studio-v3/static/; d.outputUrls.demoUrl and d.outputUrls.projectUrl are JS property paths -->
- [ ] **T.P1.14** — **Ekko** deploy S3 + S1 with `FACTORY_REAL_BUILD=1` at 100 % on first push (OQ-1: no canary, internal users). Run post-deploy smoke per rule 17: staging `POST /build` with canned session → expect real `projectId` + shortcode within 5 min; prod smoke read-only (observe next real user session end-to-end). Rollback via `scripts/deploy/rollback.sh` if prod smoke fails. estimate_minutes: 45. Files: deploy pipeline only (no source changes). DoD: staging + prod revisions serving 100 % traffic with flag on; staging smoke green and logged; prod smoke either green or auto-rollback triggered. Depends on: T.P1.12, T.P1.13b, T.P1.E1, T.P1.E2. Blocks: T.P1.16. <!-- orianna: ok -- scripts/deploy/rollback.sh is a strawberry-agents infrastructure script reference; POST /build is an HTTP route -->
- [ ] **T.P1.15** — **Viktor** follow-up (explicitly outside P1 ship gate, tracked for one week post-deploy): remove the mock `_run_build_job` branch and `FACTORY_REAL_BUILD` flag; delete now-dead mock fixtures. estimate_minutes: 20. Files: `company-os/tools/demo-factory/main.py`. DoD: flag-off path deleted; all tests still green with flag absent. Depends on: T.P1.14 + 7-day soak. Blocks: none. <!-- orianna: ok -- work-workspace path company-os/tools/demo-factory/main.py; _run_build_job is a Python identifier -->
- [ ] **T.P1.16** — **Akali** Playwright MCP QA per rule 16: session create → S2 chat → click Build → wait → observe "Demo ready" panel → click "Open iPad demo" → verify new tab loads with 200 and brand-matching content. Video + screenshots; QA report at `assessments/qa-reports/2026-04-22-p1-factory-build.md`; PR body links via `QA-Report:` line. estimate_minutes: 45. Files: `assessments/qa-reports/2026-04-22-p1-factory-build.md` (new). DoD: QA report covers all four checkpoints (create, build-click, panel-render, demo-URL-load); linked in PR body; rule 16 satisfied. Depends on: T.P1.14. Blocks: PR merge. <!-- orianna: ok -- prospective strawberry-agents path assessments/qa-reports/; QA-Report: is a PR body marker literal -->

### Totals

Total: **22 tasks** (T.P1.0, 1, 1b, 2, 3a, 3b, 4, 5a, 5b, 6, 7, 8, 9, 10a, 10b, 11, 12, 13a, 13b, 14, 15, 16, E1, E2 — 24 rows counting ops). All within 60-min cap. Sum estimate ≈ 775 min (original 545 min + decomposition overhead + env-var ops + fallback fetch + test scaffolding).

### Critical path

T.P1.0 → T.P1.1/T.P1.1b → T.P1.3a → T.P1.3b → T.P1.4 → T.P1.6 → T.P1.5a → T.P1.5b → T.P1.7 → T.P1.12 → T.P1.14 → T.P1.16. Wall-clock-critical chain ≈ 440 min assuming Phase B/C stream in parallel and T.P1.11/T.P1.E1/T.P1.E2 execute off the critical path.

### Dependency summary

- **T.P1.0 blocks:** T.P1.1, T.P1.1b, T.P1.7, T.P1.12, T.P1.3a
- **T.P1.11 blocks:** T.P1.9, T.P1.10a (S1 side cannot write new session fields without allowlist)
- **S3 integration gate:** T.P1.5a requires T.P1.1, T.P1.2, T.P1.3b, T.P1.4, T.P1.6 all green
- **S1 integration gate:** T.P1.10a requires T.P1.6 (event shape), T.P1.9 (sync writer), T.P1.11 (allowlist)
- **Deploy gate:** T.P1.14 requires T.P1.12 green + T.P1.13b merged + T.P1.E1/T.P1.E2 env vars provisioned
- **Merge gate:** T.P1.16 QA report required for PR body (rule 16)
