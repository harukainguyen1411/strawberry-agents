...
| **B4c** | `apps/myapps/package-lock.json` — `vite`+`rollup`+`esbuild` build toolchain | vite → patched, rollup → patched, esbuild → 0.25.0. **No-op 2026-04-18** — absorbed by B16a (PR #141); rollup@4.59.0 and esbuild@0.25.0 already meet targets; e9f1b25 pre-landed vite ^7.3.2 in package.json so independent B4c bump not feasible. | **Med** — build toolchain bump; watch for plugin compat. Dev-only, so runtime blast radius is zero. | M |
