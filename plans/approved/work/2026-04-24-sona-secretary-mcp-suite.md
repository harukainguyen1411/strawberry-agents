...
- [ ] **P1-T2** — Migrate Slack user token to `slack-user-token.age`. Read current `mcps/slack/.env`, age-encrypt the `SLACK_USER_TOKEN` value into `secrets/work/encrypted/slack-user-token.age`, rewrite `mcps/slack/scripts/start.sh` to use the T0-chosen decrypt pattern, delete `mcps/slack/.env`, verify with `claude mcp list` that `slack` still Connects. estimate_minutes: 35. Files: `secrets/work/encrypted/slack-user-token.age`, `mcps/slack/scripts/start.sh`, `mcps/slack/.env` (deleted), `secrets/work/REVOCATION.md`. DoD: `.env` gone; blob exists; `claude mcp list` shows slack Connected; `slack_list_channels` probe returns > 0 channels. Owner: Ekko. PR-B. BlockedBy: T0, T1.
...
- [ ] **P1-T8** — Rewrite `mcps/gdrive/scripts/start.sh` to decrypt both blobs, write their plaintext into the paths `GDRIVE_OAUTH_PATH` and `GDRIVE_CREDENTIALS_PATH` point to (the node implementation reads from files, not env — this is a special case; use `tools/decrypt.sh --target` into a tmpfile under `/tmp` with 600 perms, trap-cleanup on exit, OR refactor the node MCP to accept env vars — Ekko chooses lowest-friction). Delete the two plaintext JSONs from `mcps/gdrive/`. Verify with `list_files` probe. estimate_minutes: 50. Files: `mcps/gdrive/scripts/start.sh`, `mcps/gdrive/gcp-oauth.keys.json` (deleted), `mcps/gdrive/.gdrive-server-credentials.json` (deleted). DoD: both plaintext JSONs gone; start.sh decrypts at boot; probe succeeds. Owner: Ekko. PR-C. BlockedBy: T7a, T7b. **Note:** this is the single largest task in Phase 1 because the node MCP reads credentials from file paths, not env. If Ekko opts for refactor-to-env, split into T8a (refactor, ~30 min) + T8b (start.sh rewrite, ~20 min) to stay under 60 min.
...
- [ ] **P1-T15** — Encrypt the Gmail OAuth refresh token and rewire `mcps/gmail/scripts/start.sh` to decrypt-on-boot (same pattern as gdrive T8 if the upstream reads from file, or env-var if the upstream accepts env). Delete the plaintext token file. estimate_minutes: 35. Files: `secrets/work/encrypted/gmail-oauth.age`, `mcps/gmail/scripts/start.sh`, plaintext token file (deleted), `secrets/work/REVOCATION.md`. DoD: plaintext gone; blob exists; start.sh decrypts; Gmail probe still succeeds. Owner: Ekko. PR-E. BlockedBy: T14-Duong.
