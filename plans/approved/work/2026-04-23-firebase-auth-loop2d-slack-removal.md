...
- [ ] **T.COORD.2** — Xayah writes the test-plan stubs enumerated in Test plan. estimate_minutes: 30
- [ ] **T.COORD.3** — Ekko + Azir audit S3/S4 for `POST /session` callers (OQ-6.a). estimate_minutes: 20
- [ ] **T.COORD.4** — Duong answers §11 gating questions before promotion. estimate_minutes: 15
- [ ] **T.COORD.5** — Sibling slack-triage PR: delete `create_demo_studio_session` and every caller (OQ-1.a). Must merge and deploy before W3. estimate_minutes: 30
- [ ] **T.COORD.6** — Senna / Lucian reviews per-wave PRs. estimate_minutes: 45
- [ ] **T.COORD.7** — Akali runs the W6 Playwright E2E matrix against staging then prod. estimate_minutes: 45

### Wave 0 — Pre-flight slack-triage coordination

- [ ] **T.W0.1** — Ship the slack-triage cleanup PR (T.COORD.5) to prod. Confirm `DEMO_STUDIO_URL/session` is no longer called from slack-triage logs (24h observation window). estimate_minutes: 15. Files: slack-triage/main.py. DoD: slack-triage logs show zero `POST /session` calls for a rolling 24h; W3 unblocked.

### Wave 1 — Drop slack fields from write path

- [ ] **T.W1.1** — Xfail `tests/test_session_create_no_slack_fields.py` covering Test-plan W1 cases. owner: Soraka. estimate_minutes: 15. Files: tests/test_session_create_no_slack_fields.py. DoD: 3 xfails (kwargs-only signature; no slack keys on new doc; legacy docs still readable).
- [ ] **T.W1.2** — `session.py` — change `create_session` signature from `(slack_user_id, slack_channel, slack_thread_ts, *, owner_uid, owner_email)` to `(*, owner_uid, owner_email)`. Remove slack fields from the written doc dict. owner: Seraphine. estimate_minutes: 15. Files: session.py. DoD: T.W1.1 xfails 1–2 flip green.
- [ ] **T.W1.3** — `session_store.py` — remove `slack_user_id`/`slack_channel`/`slack_thread_ts` from `SessionRecord` and `AgentInitMetadata` dataclasses. Update `write_session_record` to not persist them. Reader (`_doc_to_session_record`) stays tolerant of legacy docs carrying the fields (ignores them). owner: Seraphine. estimate_minutes: 20. Files: session_store.py. DoD: T.W1.1 xfail 3 flips green; `mypy` / type-check clean.
- [ ] **T.W1.4** — Update `tests/test_session.py`, `tests/test_session_store_crud.py`, `tests/test_session_store_types.py`, `tests/test_session_store_no_config_write.py`, `tests/test_session_create_schema.py` to drop slack-arg usage. owner: Soraka. estimate_minutes: 15. Files: tests/test_session.py, tests/test_session_store_crud.py, tests/test_session_store_types.py, tests/test_session_store_no_config_write.py, tests/test_session_create_schema.py. DoD: tests pass with new kwargs-only signature.

### Wave 2 — UI entrypoint returns direct session URL

- [ ] **T.W2.1** — Xfail `tests/test_session_new_ui_empty_body.py` for the Test-plan W2 case. owner: Soraka. estimate_minutes: 10. Files: tests/test_session_new_ui_empty_body.py. DoD: 2 xfails (empty body → 201; studioUrl is `/session/{sid}`).
- [ ] **T.W2.2** — `main.py` — relax `NewSessionRequest.brand` / `.market` to `str | None = None`. owner: Viktor. estimate_minutes: 5. Files: main.py. DoD: empty POST body validates.
- [ ] **T.W2.3** — `main.py::create_new_session_ui` — remove `slack_user_id="ui", slack_channel="ui", slack_thread_ts="ui"` placeholder kwargs; rewrite `studio_url = f"/auth/session/{new_session_id}?token={token}"` to `studio_url = f"/session/{new_session_id}"`; remove the `token = generate_session_token(...)` call. owner: Viktor. estimate_minutes: 10. Files: main.py. DoD: T.W2.1 xfails flip green.

### Wave 3 — Delete legacy endpoints, cookies, and flag

- [ ] **T.W3.1** — Xfail `tests/test_legacy_auth_removed.py` for the Test-plan W3 cases. owner: Soraka. estimate_minutes: 15. Files: tests/test_legacy_auth_removed.py. DoD: 4 xfails (GET /auth/session/{sid} → 404; POST /session → 404; legacy cookie → 401; flag symbol absent).
- [ ] **T.W3.2** — `main.py` — delete `POST /session` handler + `SessionCreateRequest` model. owner: Viktor. estimate_minutes: 10. Files: main.py. DoD: route returns 404; model symbol gone.
- [ ] **T.W3.3** — `main.py` — delete `GET /auth/session/{sid}` handler (auth_exchange). owner: Viktor. estimate_minutes: 10. Files: main.py. DoD: route returns 404.
- [ ] **T.W3.4** — `auth.py` — delete `create_session_cookie`, `verify_session_cookie`, `generate_session_token`, `verify_and_consume_token`, `_is_legacy_user`, `AUTH_LEGACY_COOKIE_ALLOWED` flag. Remove the legacy-cookie branch from `require_user`. owner: Viktor. estimate_minutes: 20. Files: auth.py. DoD: module namespace is Firebase-only; no `legacy:` prefix anywhere.
- [ ] **T.W3.5** — Delete the `demo-studio-used-tokens` collection writes (part of `verify_and_consume_token`). No reader remains. owner: Viktor. estimate_minutes: 5. Files: auth.py. DoD: no call site writes to the used-tokens collection.
- [ ] **T.W3.6** — Flip T.W3.1 xfails green; update `tests/test_route_auth_matrix.py` regression matrix to match. owner: Soraka. estimate_minutes: 15. Files: tests/test_legacy_auth_removed.py, tests/test_route_auth_matrix.py. DoD: all deleted-route rows assert 404; cookie-only rows assert 401.
- [ ] **T.W3.7** — Remove dead-code callers and imports exposed by the W3 deletions (e.g. `COOKIE_MAX_AGE` if solely used by legacy cookie minting). owner: Viktor. estimate_minutes: 10. Files: auth.py, main.py. DoD: `ruff` / `flake8` clean; `pytest` green.

### Wave 4 — Read-site cleanup

- [ ] **T.W4.1** — Xfail `tests/test_managed_sessions_no_slack_fields.py` for the Test-plan W4 case. owner: Soraka. estimate_minutes: 10. Files: tests/test_managed_sessions_no_slack_fields.py. DoD: 1 xfail covering enrichment block.
- [ ] **T.W4.2** — `main.py::managed_sessions_list` — delete `"slackChannel": fs_doc.get("slackChannel")` and `"slackThreadTs": fs_doc.get("slackThreadTs")` from the enrichment dict. owner: Viktor. estimate_minutes: 5. Files: main.py. DoD: T.W4.1 xfail flips green.
- [ ] **T.W4.3** — `api/content-gen.yaml` — delete the `POST /session` path entry; add a `POST /session/new` entry documenting the empty-body / ownerUid-stamped contract. owner: Viktor. estimate_minutes: 20. Files: api/content-gen.yaml. DoD: OpenAPI spec validates; no `slackUserId`/`slackChannel`/`slackThreadTs` token remains in the file.
- [ ] **T.W4.4** — `scripts/smoke-test.sh` — rewrite both `POST /session` invocations (lines 85 and 89) to hit `POST /session/new` after acquiring a Firebase-emulator ID token via `curl` (per OQ-5.a). owner: Ekko. estimate_minutes: 30. Files: scripts/smoke-test.sh. DoD: smoke test green against staging; no `slackUserId`/`slackChannel` tokens remain.

### Wave 5 — Landing page UI

- [ ] **T.W5.1** — Xfail `tests/test_new_session_button.py` for the Test-plan W5 integration case. owner: Soraka. estimate_minutes: 15. Files: tests/test_new_session_button.py. DoD: 1 xfail (end-to-end `/auth/login` → `/session/new` → `/session/{sid}` 200).
- [ ] **T.W5.2** — `static/auth.js` — add `createNewSession()` export per §8. owner: Rakan. estimate_minutes: 15. Files: static/auth.js. DoD: module exports `createNewSession`; POSTs to `/session/new`; `credentials: 'include'`; redirects on 201.
- [ ] **T.W5.3** — `static/index.html` — delete paste-session-ID row, `goToSession()` inline script, and event listener; delete "Sessions are created via Slack" copy. Add `#new-session-row` with `#new-session-btn` (hidden by default); wire to `createNewSession()`; wire `onAuthReady` to reveal the row on sign-in. owner: Rakan. estimate_minutes: 20. Files: static/index.html. DoD: landing page shows Sign in for unauthed; shows Sign out + New session for authed; no session-ID input element; `#new-session-error` surface renders failures.
- [ ] **T.W5.4** — `static/studio.css` — add `.primary-btn` rule (~15 lines) matching existing auth-chrome visual language. owner: Rakan. estimate_minutes: 10. Files: static/studio.css. DoD: button renders with consistent padding / hover / focus states.
- [ ] **T.W5.5** — Flip T.W5.1 xfail green. owner: Soraka. estimate_minutes: 5. Files: tests/test_new_session_button.py. DoD: integration test passes end-to-end.

### Wave 6 — Deploy + QA

- [ ] **T.W6.1** — Deploy to staging; smoke test (T.W4.4) green; manual Akali pass on staging landing page. owner: Ekko. estimate_minutes: 15. Files: (deploy only). DoD: staging revision active; `/auth/session/ses_probe?token=probe` returns 404; `/` renders New session button for authed @missmp.tech users.
- [ ] **T.W6.2** — Akali Playwright E2E spec `test_new_session_signed_in.spec.ts` per Test-plan W6.a. owner: Akali. estimate_minutes: 15. Files: tests/e2e/test_new_session_signed_in.spec.ts. DoD: test green against staging.
- [ ] **T.W6.3** — Akali Playwright E2E spec `test_new_session_signed_out.spec.ts` per Test-plan W6.b. owner: Akali. estimate_minutes: 15. Files: tests/e2e/test_new_session_signed_out.spec.ts. DoD: test green.
- [ ] **T.W6.4** — Akali Playwright E2E spec `test_slack_handoff_dead.spec.ts` per Test-plan W6.c. owner: Akali. estimate_minutes: 10. Files: tests/e2e/test_slack_handoff_dead.spec.ts. DoD: test green.
- [ ] **T.W6.5** — Duong approves staging; Ekko deploys to prod; post-deploy smoke (`/auth/config` + `/session/new` unauth 401 + `/auth/session/*` 404). owner: Ekko. estimate_minutes: 10. Files: (deploy only). DoD: prod revision live; smoke green; rollback script on standby per Strawberry Rule 17.
