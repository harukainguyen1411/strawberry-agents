...
The five existing entries already fit this body structure largely; the migration (§Tasks G1 T1) adds frontmatter and normalizes section headings where needed.
...
Alternatively, `sync-shared-rules.sh` could resolve includes recursively to arbitrary depth. Depth-2 is sufficient for this ADR; depth-N is deferred to a follow-up if/when a second nested include is needed. §Tasks G3 T7a/T7b implement depth-2; §OQ2 gates the question of depth-N. <!-- orianna: ok -->
...
Task groups map 1:1 to the five decision layers in §2. Each group may be scheduled independently; inter-group dependencies are stated per task. Total: 16 tasks (T7 split into T7a xfail + T7b impl per CLAUDE.md rule 12; T13 split into T13a skill author + T13b sibling-ADR edit), ~620 minutes.

Conventions (per `_shared/breakdown.md` D1A):
- `owner_role:` names the executor tier — `quick-executor` (Talon/Karma quick-lane), `builder` (Talon/Jayce/Vi/Seraphine), `architect-builder` (Viktor for cross-cutting impl), `docs` (Senna/Talon for docs-only), `coordinator` (Evelynn — meta acts). Evelynn routes by tier.
- All `estimate_minutes:` ≤ 60 (D1A invariant). Tasks larger than 60 are split.
- `Depends on:` is explicit per task; group-level preambles are summary only.
- Schema-touching tasks (T2, T3, T4, T5) consume §D1/§D3 column shapes by reference; **none of them shift columns**. §D12 (Karma quick-lane, already landed) narrows §D3's INDEX template into a four-field bind contract for the dashboard — these tasks honour that contract by reference.

**Group 1 — WRITE layer (data model + schema validator).** 3 tasks, 115 minutes. Depends on: nothing. Unlocks: G2, G3.

- [ ] **T1** — Migrate the five existing ad-hoc entries to the §D1 schema. Add frontmatter and normalize body sections to `## What went wrong / ## Suggestion / ## Why I'm writing this now` where needed. No content rewriting. kind: chore. owner_role: quick-executor (Talon). estimate_minutes: 30. Depends on: nothing. Files: `feedback/2026-04-21-orianna-signing-latency.md`, `feedback/2026-04-21-orianna-signing-followups.md`, `feedback/2026-04-21-phase-discipline-approved-vs-in-progress.md`, `feedback/2026-04-21-viktor-context-ceiling-batched-impl.md`, `feedback/2026-04-22-coordinator-verify-qa-claims.md`. DoD: all five files pass `scripts/feedback-index.sh --check` (delivered in T2); existing `related_feedback:` link established between `orianna-signing-latency` and `orianna-signing-followups`. Schema contract: §D1 frontmatter fields and `state: open` default; do not invent additional columns. <!-- orianna: ok -->

- [ ] **T2** — Implement `scripts/feedback-index.sh` — POSIX-portable bash with a Node shim for YAML parsing. Reads `feedback/*.md` (not `feedback/archived/`), extracts §D1 frontmatter, writes `feedback/INDEX.md` per the §D3 template. Supports `--check` mode that validates schema without writing. kind: impl. owner_role: builder (Talon — POSIX bash + small Node shim is in-scope; if the Node shim grows past ~60 LoC of parsing logic, escalate to architect-builder Viktor). estimate_minutes: 55. Depends on: nothing (T1's migrated entries serve as fixtures but the `--check` failure paths use synthetic fixtures so T1 is not a hard prereq). Files: `scripts/feedback-index.sh` (new), `scripts/_lib_feedback_parse.mjs` (new — Node YAML parser shim modeled on `scripts/_lib_plan_structure.sh`). DoD: script renders a valid INDEX from the five migrated entries; `--check` passes on all five; `--check` fails on deliberately-malformed fixtures (missing `severity`, invalid `category`, missing `## What went wrong` section) with the faulty field named in stderr; running twice produces zero diff (Invariant 4). Schema contract: emit columns exactly per §D3 — `Severity | Date | Author | Category | Slug | Cost (min)` — and the four trailing summary lines (`Open: N | High: H | Medium: M | Low: L`, `Graduated (this week): N`, `Stale (pending prune): N`); do NOT add columns — §D12's bind contract treats `Severity`, `Date`, and the two named summary lines as breaking-change-locked. <!-- orianna: ok -->

- [ ] **T3** — Install pre-commit hook `scripts/hooks/pre-commit-feedback-index.sh` that detects `feedback/*.md` diffs and regenerates INDEX. Wire into `scripts/install-hooks.sh` alongside existing hooks. kind: impl. owner_role: builder (Talon). estimate_minutes: 30. Depends on: T2. Files: `scripts/hooks/pre-commit-feedback-index.sh` (new), `scripts/install-hooks.sh` (edit). DoD: hook runs on a test commit that edits a feedback file — INDEX is regenerated and staged in the same commit; hook fails loud on a commit with malformed frontmatter pointing at §D1; `--no-verify` is NOT used anywhere in the hook (CLAUDE.md rule 14); hook is registered in `scripts/install-hooks.sh` and a fresh `bash scripts/install-hooks.sh` in a clean clone wires it without manual steps. <!-- orianna: ok -->
...
- [ ] **T4** — Add boot-chain read in Evelynn + Sona `CLAUDE.md` startup sections per §D3. One line each: "Read `feedback/INDEX.md` — first 20 lines. If `count_open_high > 0`, surface top-3 high entries to Duong at first opportunity." kind: docs. owner_role: docs (Talon). estimate_minutes: 35. Depends on: T2 (INDEX file must be generatable; an empty/missing INDEX should not break boot — verify graceful degradation). Files: `agents/evelynn/CLAUDE.md` (edit), `agents/sona/CLAUDE.md` (edit). DoD: fresh coordinator session (both Evelynn and Sona spawn paths) reads INDEX as part of startup; high-severity count is surfaced in the first user turn when >0; when =0 a one-line acknowledgement appears in the startup recap rather than silence; missing-INDEX path emits a single warning and does not abort startup. Schema contract: boot text refers to the §D3 column names (`count_open_high`, derived from the `Open: N | High: H | …` summary line) by name only — does not duplicate the column list, consistent with §D12's bind contract. <!-- orianna: ok -->

- [ ] **T5** — Extend `.claude/agents/skarner.md` with the `feedback-search` query kind per §D3 on-demand route. Grammar: `feedback-search <category|severity|author|keyword>`. Search targets: `feedback/INDEX.md` first as the fast path, then `feedback/*.md` frontmatter+body for keyword matches, then `feedback/archived/*.md` as a last resort when the caller supplies `--include-archived`. kind: docs. owner_role: docs (Talon). estimate_minutes: 40. Depends on: T1 (real entries to demo against) + T2 (INDEX exists). Files: `.claude/agents/skarner.md` (edit). DoD: Skarner's agent def lists `feedback-search` in its query-kind table with example dispatches; a dry-run query for `feedback-search review-loop` returns the orianna-signing cluster; the four named filter dimensions (`category | severity | author | keyword`) match the §D1 frontmatter field names and §D3 column names exactly. Resolves §OQ5. <!-- orianna: ok -->

**Group 3 — ENCOURAGE layer (shared-rules inline across all paired agents).** 4 tasks, 125 minutes. Depends on: G1 complete (so an entry written from the trigger has a valid home). Unlocks: universal text-level reach.

- [ ] **T6** — Author `.claude/agents/_shared/feedback-trigger.md` per §D4 content (~35 lines). Contains: the 7 triggers, the two-mode invocation contract (coordinator-commit vs subagent-defer-for-sweep), the "do NOT write for" list, the 3-per-session budget + Lux escalation, and the Skarner on-demand query pointer. kind: docs. owner_role: docs (Talon). estimate_minutes: 35. Depends on: nothing (content is fully specified in §D4). Files: `.claude/agents/_shared/feedback-trigger.md` (new). DoD: file exists at target length (~35 lines, ±10); `scripts/lint-subagent-rules.sh` recognizes it as a valid shared file; body matches §D4 content (7 triggers, Mode A/B contract, 3-per-session budget, Skarner pointer). <!-- orianna: ok -->

- [ ] **T7a** — Author the xfail test for depth-2 nested-include resolution in `scripts/__tests__/sync-shared-rules.xfail.bats`. Cases: (a) depth-2 idempotency on clean tree → zero diff after two runs; (b) one-line edit to `_shared/feedback-trigger.md` propagates to all 10 `_shared/<role>.md` files AND to every paired agent def carrying the role include; (c) depth-3 include emits a clear error referencing §OQ2. kind: test. owner_role: test-impl (Xayah authors test plan; Talon implements xfail bats). estimate_minutes: 30. Depends on: T6. Files: `scripts/__tests__/sync-shared-rules.xfail.bats` (edit). DoD: tests committed as xfail (CLAUDE.md rule 12 — xfail before impl); test names reference `plans/approved/personal/2026-04-21-agent-feedback-system.md` per the rule.

- [ ] **T7b** — Implement depth-2 resolution in `scripts/sync-shared-rules.sh` (§D4.1). Add pass-2 scanner for `<!-- include: _shared/feedback-trigger.md -->` markers in already-expanded content. kind: impl. owner_role: builder (Talon). estimate_minutes: 30. Depends on: T7a (xfail must be committed first). Files: `scripts/sync-shared-rules.sh` (edit). DoD: T7a tests flip from xfail to pass; running sync twice on a clean tree produces zero diff (Invariant 5); a paired agent def ends up with both its role shared content AND the feedback-trigger stanza inlined below it; depth-3+ includes produce a clear error pointing at §OQ2.

- [ ] **T8** — Add `<!-- include: _shared/feedback-trigger.md -->` marker to the bottom of each of the 10 existing `_shared/<role>.md` files (`ai-specialist`, `architect`, `breakdown`, `builder`, `frontend-design`, `frontend-impl`, `quick-executor`, `quick-planner`, `test-impl`, `test-plan`); re-run `scripts/sync-shared-rules.sh`; commit the resulting diff across every paired `.claude/agents/*.md`. kind: docs. owner_role: docs (Talon — mechanical edit + sync script invocation). estimate_minutes: 30. Depends on: T7b. Files: all 10 files under `.claude/agents/_shared/` (edit) + the sync-produced diff on paired agent defs. DoD: `scripts/lint-subagent-rules.sh` passes; spot-check of 3 random agent defs (e.g. `.claude/agents/jayce.md`, `.claude/agents/vi.md`, `.claude/agents/karma.md`) confirms the feedback-trigger stanza is present below the role stanza; lint check (per §D4.2) verifies each `_shared/<role>.md` carries exactly one feedback-trigger include marker. <!-- orianna: ok -->
...
- [ ] **T9** — Author `.claude/skills/agent-feedback/SKILL.md` — the atomic write primitive per §D4A.1. Detects caller-type (coordinator-or-Lissandra vs subagent) and branches commit behavior: mode A commits immediately (`chore: feedback — <slug>`), mode B leaves file uncommitted for sweep. Includes the four-field prompt (category, severity, friction-cost-minutes, what-went-wrong+suggestion free-form), filename derivation per §D1 (Asia/Bangkok local time), frontmatter synthesis, and the three-section body structure. kind: docs. owner_role: architect-builder (Viktor — caller-type detection branching is non-trivial; if Talon takes it, route via builder + Senna review). estimate_minutes: 60. Depends on: T2 (schema validator must exist so the skill can rely on `--check` behaviour). Files: `.claude/skills/agent-feedback/SKILL.md` (new). DoD: dry-run against a mocked coordinator trigger produces a valid §D1 file + commit; dry-run against a mocked subagent trigger produces a valid §D1 file with no commit; caller-type detection falls back safely (unknown → mode B, no commit); the four prompted fields map 1:1 to §D1 frontmatter (`category`, `severity`, `friction_cost_minutes`, free-form synthesized into the body's three required sections); commit prefix is `chore: feedback — <slug>` for mode A (matches diff-scope rule per CLAUDE.md rule 5). Skill name `agent-feedback` (not `feedback`) to avoid collision with Claude Code's reserved `/feedback` built-in. <!-- orianna: ok -->

- [ ] **T10** — Edit `.claude/skills/end-session/SKILL.md` per §D4A.2 to insert the seven-trigger reflection step before the final commit, AND the coordinator fallback sweep per §D4A.5 at the very end (picks up any `feedback/*.md` left uncommitted by a subagent session that never reached `/end-subagent-session`). Explicit null-output rule: when all triggers clean, log "no feedback this session (all triggers clean)" in the handoff shard — silence is not acceptance. kind: docs. owner_role: docs (Talon). estimate_minutes: 40. Depends on: T9. Files: `.claude/skills/end-session/SKILL.md` (edit). DoD: reflection step text matches §D4A.2 body; sweep step is idempotent against zero uncommitted files; sweep commit uses `chore: feedback sweep — <coordinator> — <date>` prefix; skill verifies each swept entry's own `author:` frontmatter is preserved (sweep-author is coordinator; entry-authors are original subagents — Invariant 2 preserved). <!-- orianna: ok -->

- [ ] **T11** — Edit `.claude/skills/pre-compact-save/SKILL.md` per §D4A.3 and cross-reference in `.claude/agents/lissandra.md`. Insert the seven-trigger reflection step before Lissandra's final consolidation commit. Voice invariant: feedback's `author:` is the coordinator Lissandra is impersonating (`evelynn` or `sona`), never `lissandra` — Invariant 2. kind: docs. owner_role: docs (Talon). estimate_minutes: 40. Depends on: T9. Files: `.claude/skills/pre-compact-save/SKILL.md` (edit), `.claude/agents/lissandra.md` (edit). DoD: reflection step present in both files; voice invariant stated in both with a cross-reference to Invariants §2; dry-run where Lissandra is mid-compact for Evelynn produces a feedback entry with `author: evelynn`. <!-- orianna: ok -->

- [ ] **T12** — Edit `.claude/skills/end-subagent-session/SKILL.md` to add the §D4A.5 subagent sweep step. Picks up any `feedback/*.md` files the subagent wrote mid-task via mode-B `/agent-feedback` and commits them in a single `chore: feedback sweep — <agent> — <date>` commit after the normal handoff/learnings commit, before the final session-exit report. Fallback: if this skill is not reached (session crash/timeout), the dispatching coordinator's `/end-session` sweep (T10) is the safety net. kind: docs. owner_role: docs (Talon). estimate_minutes: 40. Depends on: T9. Files: `.claude/skills/end-subagent-session/SKILL.md` (edit). DoD: sweep step present; `git status --porcelain | grep '^?? feedback/'` detection logic matches §D4A.5 body; idempotent when zero uncommitted feedback files exist (no-op, no log); double-sweep safety verified (subagent sweep then coordinator sweep finds nothing — no duplicate commit). <!-- orianna: ok -->

**Group 5 — CONSOLIDATE layer (weekly digest via audit piggyback).** 3 tasks, 115 minutes. Depends on: G1 complete + G4 complete (real entries must be emittable before a digest has anything to chew on) + a landed daily-agent-repo-audit Routine for T13b (sibling ADR §D10 edit target exists).

- [ ] **T13a** — Author `.claude/skills/feedback-consolidate/SKILL.md` — the Sunday dispatch skill Lux + Karma execute. Contains: Lux-categorize dispatch template (group by category + similarity, flag `graduate`/`keep-open`/`stale`), Karma-triage dispatch template (one-paragraph problem + one-paragraph candidate solution + proposed owner for each `graduate` entry), and the §D5 digest schema. Skill name explicitly `feedback-consolidate` (not `feedback` — built-in collision; not `agent-feedback` — T9 write-primitive collision). kind: docs. owner_role: docs (Talon — content fully specified in §D5). estimate_minutes: 50. Depends on: nothing (independent of audit-routine landing). Files: `.claude/skills/feedback-consolidate/SKILL.md` (new). DoD: skill body matches §D5 templates verbatim; token-budget note (~50k in / ~12k out) retained; Invariant 7 stated explicitly — consolidate writes only to `assessments/feedback-digests/**` (never mutates `feedback/*.md` bodies, only frontmatter `state:`).

- [ ] **T13b** — Edit the audit-routine ADR's parent-prompt §D10 to add the Sunday conditional block per §D7 (dispatch Lux-categorize + Karma-triage additionally when `day-of-week == 0`, consume outputs into `assessments/feedback-digests/$DATE.md`, mutate feedback `state:` on `graduate`/`stale` verdicts, include digest summary in commit + Evelynn inbox message). Cross-reference `architecture/audit-routine.md` (created by the sibling ADR's T4). kind: docs. owner_role: docs (Talon). estimate_minutes: 30. Depends on: T13a + sibling audit-routine ADR landed at `plans/in-progress/` or later. Files: `plans/proposed/personal/2026-04-21-daily-agent-repo-audit-routine.md` §D10 (edit), `architecture/audit-routine.md` (edit pending sibling T4). DoD: Sunday conditional block exists in sibling §D10 and references `.claude/skills/feedback-consolidate/SKILL.md`; cross-reference to `architecture/audit-routine.md` is present (or marked `pending sibling T4` if that file does not yet exist). <!-- orianna: ok -->

- [ ] **T14** — First live digest dry-run against the five migrated entries + any net-new week-one entries. Lux + Karma produce `assessments/feedback-digests/<first-sunday-after-launch>.md`. Evelynn + Duong review; pattern locks in or digest schema amendment graduates to a follow-up edit to this ADR. kind: meta. owner_role: coordinator (Evelynn dispatches Lux + Karma; Evelynn consolidates and commits). estimate_minutes: 35. Depends on: T13b. Files: `assessments/feedback-digests/<date>.md` (new). DoD: digest follows §D5 schema; identifies the Orianna-signing cluster (entries 1 + 2 from the migration set) as `graduate` with a named candidate ADR stub pointing at recommendation A of `feedback/2026-04-21-orianna-signing-latency.md`; at least one `keep-open` and one `stale` or `low` entry disposition is exercised so the pipeline's non-graduate paths are verified live. <!-- orianna: ok -->
...
T1 ──┐
     ├──► T5 ──► (broadcast live)
T2 ──┼──► T3, T4
     └──► T9 ──► T10, T11, T12 ──► (ritual live)
T6 ──► T7a ──► T7b ──► T8 ──► (encourage live)
T13a ──► T13b ──► T14 ──► (consolidate live)
```

Phase gates:
- **G1 gate** (after T3): pre-commit hook live, schema is enforceable.
- **G2 gate** (after T5): coordinators see INDEX; Skarner serves on-demand.
- **G3 gate** (after T8): every paired agent def carries the trigger stanza.
- **G4 gate** (after T12): all four ritual integration points wired (skill + 3 session-lifecycle edits).
- **G5 gate** (after T14): one full Sunday digest cycle verified live.

### Schema-stability advisory (consumed by §D12 dashboard bind contract)

Karma's parallel quick-lane has landed §D12 (Dashboard schema-bind points), which narrows §D3's INDEX template into an explicit four-field read contract for `plans/approved/personal/2026-04-25-retrospection-dashboard-and-canonical-v1.md` §Q7. (Note: the dashboard plan nominates this section as "§D11"; the actual section number is §D12 because §D11 was already used for the sync-shared-rules boundary.)

This breakdown is consistent with §D12:
- T2 (`scripts/feedback-index.sh`) emits the §D3 column tuple `Severity | Date | Author | Category | Slug | Cost (min)` and the four summary lines exactly. The four §D12 bind-points (`Severity` column, `Date` column, `Open: N | High: N | Medium: N | Low: N` summary line, `Graduated (this week): N` summary line) are produced as a side-effect of conforming to §D3. **Do not rename, remove, or retype these four bind-points without paired dashboard-side update per §D12 invariant.**
- T4 (coordinator boot read) refers to bind-point fields by name only.
- T5 (Skarner `feedback-search`) filters on §D1 frontmatter fields, not on INDEX columns.
- T13a's digest schema (§D5) is orthogonal — it writes to `assessments/feedback-digests/` and never feeds the dashboard's INDEX read path.

No task in this breakdown amends `feedback/INDEX.md` columns. If a future feedback-system change wants to alter a bind-point, that change ships paired with a dashboard-side update per §D12.
...
- `scripts/__tests__/feedback-index.xfail.bats` (new) — covers invariants 1 above + the §D12 schema-bind contract.
- `scripts/__tests__/feedback-index-bind-contract.xfail.bats` (new) — dedicated §D12 dashboard read-contract surface (separated so the dashboard plan's Phase 1 tests can `bats` this file directly). <!-- orianna: ok -->
- `scripts/__tests__/pre-commit-feedback-index.xfail.bats` (new) — covers invariant 2 + Invariant 1 (one-file-writing-path) detection.
- `scripts/__tests__/sync-shared-rules.xfail.bats` (edit existing) — covers invariant 3 + Invariant 5 (depth-2 idempotency).
- `scripts/__tests__/feedback-pipeline-integration.xfail.bats` (new) — cross-task smoke: write → INDEX regen → coordinator boot read end-to-end.
- `scripts/__tests__/feedback-author-fidelity.xfail.bats` (new) — Invariant 2 author-fidelity in skill dry-runs (T9, T11).
- `scripts/__tests__/feedback-sweep-idempotency.xfail.bats` (new) — Invariants 1+7 across sweep paths (T10, T12).

These tests are xfail-first per CLAUDE.md rule 12 even though `tests_required: false`, because the repo's invariant-check pattern keeps them cheap and durable. Implementer: Rakan (complex track — schema-bind contract is cross-cutting per ADR §D12 and graph-G3 sync work touches 10+ files).

### Test tasks — xfail-first pairings (Rule 12)

Each implementation task that ships code has a paired `xfail` test task that **must commit before the impl** on the same branch (Rule 12). Tasks below are inline-additive to the §Tasks list; the dependency graph extends as `T<N-test> → T<N-impl>`. Total: 11 test tasks (~395 minutes).

- [ ] **TT2** — xfail bats for `scripts/feedback-index.sh` schema validator + `--check` mode. Five valid fixtures (the migrated §D1 entries) exit 0; three malformed fixtures (missing `severity`, invalid `category` enum value, missing `## What went wrong` section heading) exit non-zero with the faulty field named in stderr; rendering the same fixture set twice produces byte-identical INDEX (Invariant 4). Guards T2 DoD bullets 1-4. kind: test. estimate_minutes: 45. Files: `scripts/__tests__/feedback-index.xfail.bats` (new), `scripts/__tests__/fixtures/feedback/valid/*.md` (new — 5), `scripts/__tests__/fixtures/feedback/malformed/*.md` (new — 3). DoD: tests committed as xfail referencing this ADR; runner is `bats` (already in repo per `sync-shared-rules.xfail.bats` precedent). Committed before T2 per Rule 12.

- [ ] **TT2-bind** — xfail bats for the §D12 dashboard read contract. Asserts: (a) the rendered INDEX header row contains `Severity` as column 1 and `Date` as column 2 in the §D3 order — not aliases, not reordered; (b) trailing summary line `Open: N | High: N | Medium: N | Low: N` is emitted with three pipe-delimited `<key>: <int>` pairs after the `Open: N` lead and the four keys are exactly `Open|High|Medium|Low`; (c) trailing summary line `Graduated (this week): N` is emitted; (d) **schema-mutation simulation** — when a fixture INDEX template is copied into a temp dir and the renderer is invoked with `FEEDBACK_INDEX_RENAME_SEVERITY=Priority` (env override the test harness installs to simulate a future rename), the bind-contract assertion `[Severity column present]` FAILS loud — proving the test catches a bind-point breaking change before it ships. Guards §D12 bind contract for downstream `plans/approved/personal/2026-04-25-retrospection-dashboard-and-canonical-v1.md` §Q7. kind: test. estimate_minutes: 60. Files: `scripts/__tests__/feedback-index-bind-contract.xfail.bats` (new), `scripts/__tests__/fixtures/feedback/bind-mutation/*.md` (new). DoD: four assertions land green once T2 implements; the schema-mutation case is implemented as a negative assertion that runs the renderer with an env-var rename hook and confirms the bind check trips. Committed before T2 per Rule 12.

- [ ] **TT3** — xfail bats for the pre-commit hook. Three cases: (a) commit touching `feedback/2026-04-21-orianna-signing-latency.md` with valid frontmatter → INDEX is regenerated, auto-staged, commit succeeds; (b) commit touching a feedback file with malformed frontmatter → hook exits non-zero with stderr citing §D1 schema and the offending field; (c) **Invariant 1 audit** — a synthetic commit that adds `feedback/2099-01-01-rogue.md` via raw `git add` with a commit prefix other than `chore: feedback` or `chore: feedback sweep` is detected by the hook (or by an `audit` mode of `feedback-index.sh --check`) and flagged. The hook runs in a temp git repo using `scripts/install-hooks.sh` so installation is exercised. Guards T3 DoD + Invariant 1. kind: test. estimate_minutes: 45. Files: `scripts/__tests__/pre-commit-feedback-index.xfail.bats` (new). DoD: tests xfail until T3 lands; no `--no-verify` anywhere. Committed before T3 per Rule 12.

- [ ] **TT4** — Boot-chain test for coordinator INDEX read. Spawns a fresh shell with `agents/evelynn/CLAUDE.md` rendered as text and asserts the `Read feedback/INDEX.md` instruction is present at startup-step 3a (or wherever §D3 placed it); same for `agents/sona/CLAUDE.md`. Missing-INDEX path (rename `feedback/INDEX.md` aside, render boot text, ensure no abort directive) is asserted via grep absence of any `exit`, `abort`, or `fatal` token tied to the INDEX-read line. Guards T4 DoD bullets 1+3. kind: test. estimate_minutes: 25. Files: `scripts/__tests__/feedback-boot-chain.xfail.bats` (new). DoD: docs-text assertions; no actual session spawn required. Committed before T4 per Rule 12.

- [ ] **TT5** — Skarner agent-def test. Asserts `.claude/agents/skarner.md` after T5 contains `feedback-search` in its query-kind table with the four named filter dimensions matching §D1 frontmatter field names exactly (`category`, `severity`, `author`, `keyword`). Drift between this list and §D1's enum names trips the test. Guards T5 DoD. kind: test. estimate_minutes: 20. Files: `scripts/__tests__/skarner-feedback-search.xfail.bats` (new). DoD: regex check + frontmatter-field cross-reference. Committed before T5 per Rule 12.

- [ ] **TT7a** — Already declared as T7a in §Tasks (depth-2 sync xfail). This test plan promotes T7a's coverage to also assert: (d) **lint-subagent-rules** verifies each `_shared/<role>.md` carries exactly **one** `<!-- include: _shared/feedback-trigger.md -->` marker (§D4.2); duplicate-marker fixture trips the lint. Guards T7a DoD + Invariant 5 + §D4.2. kind: test. estimate_minutes: 0 (rolled into T7a; no new task — call out is documentation-only). Files: `scripts/__tests__/sync-shared-rules.xfail.bats` (edit-extend). Already committed before T7b per Rule 12 via T7a.

- [ ] **TT9** — xfail dry-run test for `/agent-feedback` SKILL.md. Two cases: (a) coordinator-mode dry-run with `CLAUDE_AGENT_NAME=evelynn` → produces a §D1-conformant file at `feedback/<derived>.md` AND a commit with prefix `chore: feedback — <slug>`; (b) subagent-mode dry-run with `CLAUDE_AGENT_NAME=jayce` → produces the file but **no** commit (working tree dirty); (c) unknown-caller (`CLAUDE_AGENT_NAME=` empty) defaults to mode B — no commit. Filename derivation tested against Asia/Bangkok timezone (force `TZ=Asia/Bangkok`) per §D1. Guards T9 DoD + Invariant 1. kind: test. estimate_minutes: 50. Files: `scripts/__tests__/agent-feedback-skill.xfail.bats` (new). DoD: skill body parsed text-level (the skill is markdown, not a script — the test asserts the skill's documented branching rules are stated and that an associated reference renderer in `scripts/feedback-skill-render.sh` produces the expected file). Committed before T9 per Rule 12.

- [ ] **TT10-12** — xfail sweep idempotency. Cases: (a) `/end-session` skill text contains the §D4A.2 reflection step and §D4A.5 fallback sweep block; (b) `/end-subagent-session` skill text contains the §D4A.5 sweep step; (c) **double-sweep safety** — given a fixture working tree with one uncommitted `feedback/2099-01-01-test.md`, the subagent sweep commits it under `chore: feedback sweep — <agent> — <date>`; running the coordinator sweep immediately after finds zero uncommitted files and produces no commit (Invariant 1 preserved); (d) **null-output rule** — when no triggers fire, the handoff shard contains the literal `no feedback this session (all triggers clean)` line (T10 DoD: silence is not acceptance). Guards T10 + T12 DoDs + Invariants 1 and 2. kind: test. estimate_minutes: 50. Files: `scripts/__tests__/feedback-sweep-idempotency.xfail.bats` (new). DoD: tests xfail; sweep harness uses a temp git repo. Committed before T10 per Rule 12.

- [ ] **TT11** — Author-fidelity (Invariant 2). Lissandra-impersonating-Evelynn dry-run: invocation context sets `CLAUDE_AGENT_NAME=lissandra` AND `LISSANDRA_IMPERSONATES=evelynn`; the resulting `feedback/*.md` MUST have `author: evelynn` (not `lissandra`); the resulting commit author trailer (if any) is consistent. Negative case: `LISSANDRA_IMPERSONATES=` unset with `CLAUDE_AGENT_NAME=lissandra` → skill ERRORS rather than defaulting to `author: lissandra` (defending against the silent-proxy violation called out in Invariant 2). Guards T11 DoD + Invariant 2. kind: test. estimate_minutes: 35. Files: `scripts/__tests__/feedback-author-fidelity.xfail.bats` (new). DoD: tests xfail until T11 cross-references invariant 2 in both `pre-compact-save/SKILL.md` and `lissandra.md`. Committed before T11 per Rule 12.

- [ ] **TT-INT** — Cross-task integration smoke. Pipeline test: in a temp git repo clone, (1) render the `_shared/feedback-trigger.md` file (T6 fixture), (2) run `sync-shared-rules.sh` to depth-2 (T7b), (3) invoke `agent-feedback` skill renderer in coordinator mode → file lands, commit succeeds, pre-commit hook auto-stages INDEX (T2+T3+T9), (4) render the coordinator boot block (T4) and verify it parses INDEX, finds `count_open_high == 1` (since the synthetic entry was high-severity), and the boot text would surface to Duong, (5) `feedback-search high` via Skarner shim returns the synthetic entry (T5). One-shot end-to-end exercises G1+G2+G3+G4 across all the seam files. Guards: integration of T2, T3, T4, T5, T7b, T9. kind: test. estimate_minutes: 60. Files: `scripts/__tests__/feedback-pipeline-integration.xfail.bats` (new). DoD: smoke runs in <30s; tests xfail until G4 gate. Committed before any G4-gating impl task per Rule 12.

- [ ] **TT-INV** — Invariant regression suite. One bats file with one test per invariant in §Invariants (10 invariants → 10 tests), each citing its invariant number. Examples: Invariant 6 (state machine monotone) — fixture entry with `state: graduated` mutated to `state: open` is rejected by `feedback-index.sh --check`; Invariant 7 (scope separation) — a synthetic patch that mutates `feedback/2099-01-01-test.md` AND `apps/strawberry/src/foo.ts` in the same commit fails an audit lint; Invariant 8 (secret-free) — fixture with an OpenAI-shaped secret is caught by the existing pre-commit secret scanner over `feedback/**` (assertion only — does not write the secret to git, uses an env-substituted placeholder); Invariant 9 (concern-tagged) — fixture missing `concern:` field fails `--check`; Invariant 10 (feedback ≠ plan) — a `git mv feedback/foo.md plans/proposed/personal/foo.md` in a synthetic commit is caught by the plan-lifecycle pre-tooluse guard (per CLAUDE.md rule 7). Guards Invariants 1-10. kind: test. estimate_minutes: 60. Files: `scripts/__tests__/feedback-invariants.xfail.bats` (new). DoD: 10 named tests, each with `# guards Invariant <N>` comment; all xfail until the relevant impl task lands. Committed before T2 per Rule 12 (the earliest-landing impl whose code these tests will eventually exercise).

### Schema-stability gaps (Xayah analysis)

Three concerns surfaced while authoring this plan; each is now covered above but called out so future plan editors do not regress:

1. **The §D12 mutation-simulation test** is critical. The bind contract is documentation-only without a test that proves a rename of `Severity → Priority` actually trips the assertion. TT2-bind §(d) exists for this reason. If TT2-bind is dropped, the dashboard plan's Phase 1 has no upstream tripwire and the contract becomes aspirational.

2. **Invariant 1 needs an audit-mode test**, not just hook coverage. The hook only runs at commit time; an entry written by raw Write that bypasses the hook (e.g. via `--no-verify` in a hostile-or-buggy agent) lands silently. TT3 case (c) exercises an audit-style scan for entries whose introducing commit prefix is not `chore: feedback*`. If T2's `feedback-index.sh --check --audit-history` mode is descoped, this gap re-opens — flag it back to Aphelios.

3. **Author-fidelity (Invariant 2) has two failure modes**, not one. Mode 1: subagent-as-coordinator-proxy (Lissandra writes `author: lissandra` instead of `evelynn`). Mode 2: coordinator-as-subagent (Evelynn invokes `/agent-feedback` and types `author: jayce` because Jayce reported the friction verbally). TT11 covers Mode 1; **Mode 2 is currently not covered** — recommend extending TT11 with a third case where the skill rejects an `author:` value that does not match `CLAUDE_AGENT_NAME` unless `LISSANDRA_IMPERSONATES` is set. Tag for follow-up if T9's skill author lands without a matching guard.
...
- **OQ6 — Does Sona read this same `feedback/INDEX.md`?** Yes, per §Tasks G2 T4. But Sona's startup chain is in `agents/sona/CLAUDE.md` (work-concern specific); the boot-chain edit in T4 lands in both coordinator CLAUDE.md files, not just Evelynn's. <!-- orianna: ok -->
