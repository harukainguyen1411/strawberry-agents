...
- **Rationale:** Synthesis ADR with clear owner (Swain), no unresolved TBDs in gating sections, and concrete coordination tasks T1–T5. §7.5 records Duong's hands-off Default-track approval of all 20 OQs with compact-form answers stamped (authority commit `c4be153b`); body referenced by directive verbatim. References to the five sister ADRs resolve by filename regardless of their current lifecycle stage (sister promotions are concurrent and the synthesis depends on their existence in the repo, not their location). `tests_required: false` is justified — implementation tests live downstream on Kayn's W1–W3 breakdown per §test-plan. `qa_plan: none` is justified for a doc-only synthesis.
- **Simplicity:** WARN: possible overengineering — eleven body sections, five primitives, eight stages, four-wave plan, ten architecture impacts; this is heavy for a doc-emission plan. The heft is partially justified by the join-point role across five ADRs, but the canonical `process.md` itself (T2, the actual deliverable) could likely be emitted from a thinner spec without the multi-stage wave plan in §6 — most of W1/W2/W3 sequencing properly belongs to the source ADRs' own implementation plans, not this synthesis. Consider whether T3 (Kayn breakdown of the wave plan) double-counts work the source ADRs already own.
