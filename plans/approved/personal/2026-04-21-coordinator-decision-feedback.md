...
- [ ] **T9** — Update `.claude/agents/evelynn.md` + `.claude/agents/sona.md`: boot chain (§6.1) + Decision Capture Protocol (§6.2) + Operating Modes addendum (§6.3). estimate_minutes: 40. Files: `.claude/agents/evelynn.md`, `.claude/agents/sona.md`. DoD: boot order §9 in both defs; Decision Capture Protocol section present with the a/b/c+Predict+Confidence template and the 3-step capture protocol; Operating Modes addendum addresses hands-on/hands-off parity.
  > **Note:** This task requires edits to `.claude/agents/evelynn.md`, which is harness-denied from subagent context (confirmed Ekko memory 2026-04-20). Execute via Evelynn-direct or Duong-manual.
- [ ] **T10** — Update `agents/evelynn/CLAUDE.md` + `agents/sona/CLAUDE.md` Startup Sequence (§6.4) + extend `agents/memory/agent-network.md` Memory Consumption section (§6.5). estimate_minutes: 25. Files: `agents/evelynn/CLAUDE.md`, `agents/sona/CLAUDE.md`, `agents/memory/agent-network.md`. DoD: §6.4 and §6.5 edits landed; boot order matches §9; subagent-facing consumption doc reads clean on a fresh pass.
  > **Note:** This task requires edits to `agents/evelynn/CLAUDE.md`, which is harness-denied from subagent context (confirmed Ekko memory 2026-04-20). Execute via Evelynn-direct or Duong-manual.
...
POSIX bash + `bats` for shell harnesses (matches `scripts/__tests__/*.xfail.bats` precedent across feedback-system, sync-shared-rules, dashboard plans). `node:test` is reserved for any future JS harness; not used in this plan because no JS surface ships. No python, no vitest. All harnesses runnable on macOS and Git-Bash-on-Windows per Rule 10.

Implementer: **Rakan (complex track)** — schema-bind contract is cross-cutting per §3.5 and ties into the retrospection-dashboard plan's Phase 1 bind tests; multi-axis rollup invariants and §6.3 hands-off separation require fixture-heavy harnesses.

### Test tasks — xfail-first pairings (Rule 12)

Each impl task that ships code has a paired `xfail` test task that **commits before the impl** on the same branch (Rule 12). The dependency graph extends as `T<N-test> → T<N-impl>`. T1, T3, T5 in the §Tasks list are the existing xfail seeds; the entries below extend coverage to the §3.5 bind contract, integration smoke, and per-invariant regression suite. Total: 8 new test tasks (~360 minutes); existing T1/T3/T5 unchanged.

- [ ] **TT1-bind** — xfail bats for the §3.5 dashboard read contract. Asserts: (a) every fixture decision-log file under `scripts/__tests__/fixtures/decisions/valid/*.md` carries the four bind-points (`axes` as YAML kebab-string list, `match` as boolean, `coordinator_confidence` as one of `low|medium|medium-high|high`, `decision_id` matching filename stem) and `validate_decision_frontmatter` passes; (b) **rename simulation** — when a fixture is copied to a temp dir and `validate_decision_frontmatter` is invoked with `DECISION_RENAME_AXES=topics` (env override that the validator's harness honours by reading the renamed key), the dashboard contract test asserter `[axes column present and named exactly "axes"]` FAILS loud, proving the test catches a future rename before it ships; (c) same mutation simulation for `match → matched`, `coordinator_confidence → coord_conf`, and `decision_id → log_id` — four FAIL-loud cases total; (d) the enum `low|medium|medium-high|high` is asserted by negative case (`coordinator_confidence: very-high` rejected with stderr citing the §3.5 bind enum); (e) `decision_id != filename_stem` rejected. Guards §3.5 bind contract for downstream `plans/approved/personal/2026-04-25-retrospection-dashboard-and-canonical-v1.md` §Q7/§Q8 coordinator drill-down. kind: test. estimate_minutes: 55. Files: `scripts/__tests__/decision-log-bind-contract.xfail.bats` (new), `scripts/__tests__/fixtures/decisions/valid/*.md` (new — 4), `scripts/__tests__/fixtures/decisions/bind-mutation/*.md` (new — 4). DoD: five assertions land green once T2 implements the validator; the four mutation cases run the validator with env-var rename hooks and confirm the bind check trips; tests xfail until T2. Committed before T2 per Rule 12.

- [ ] **TT2-rollup** — xfail bats for `rollup_preferences_counts` (§4.4) idempotency + summary-prose preservation. Three cases: (a) given a 12-file fixture across 3 axes (`scope-vs-debt` ×6, `explicit-vs-implicit` ×4, `hand-curated-vs-automated` ×2), running `rollup_preferences_counts` produces `Samples: 6 (a: 4, b: 1, c: 1)`, `Match rate: 67%`, `Confidence: medium` for `scope-vs-debt`; (b) running the rollup twice in a row produces byte-identical `preferences.md` (idempotency invariant from existing Test plan); (c) **summary-prose preservation** — given a fixture `preferences.md` with hand-curated `Summary:` paragraphs containing markdown bullets and italics, running rollup leaves the prose byte-identical while regenerating only `Samples:` / `Match rate:` / `Confidence:` / `Notable misses:` lines. Guards §4.3 step 3 + §3.2 schema rules. kind: test. estimate_minutes: 50. Files: `scripts/__tests__/decision-rollup-idempotency.xfail.bats` (new), `scripts/__tests__/fixtures/decisions/rollup-12/log/*.md` (new — 12), `scripts/__tests__/fixtures/decisions/rollup-12/preferences.before.md` (new), `scripts/__tests__/fixtures/decisions/rollup-12/preferences.expected.md` (new). DoD: tests xfail until T2 implements `rollup_preferences_counts`; diff verification uses `diff -u`. Committed before T2 per Rule 12.

- [ ] **TT4-handsoff** — xfail bats for §6.3 hands-off separation invariant. Cases: (a) fixture with 10 decisions tagged `scope-vs-debt`, of which 3 carry `coordinator_autodecided: true` and `duong_pick: hands-off-autodecide`; rollup emits `Samples: 7 (a: 4, b: 2, c: 1; +3 hands-off)` — explicit-pick numerator excludes hands-off; (b) `match_rate` percentage in the same axis is computed over the 7 explicit picks, not all 10 — assertion: `match_rate == round(matches_among_explicit / 7 * 100)`; (c) negative case: a malformed log with `duong_concurred_silently: true` AND `coordinator_autodecided: true` is rejected by `validate_decision_frontmatter` (the two flags are mutually exclusive). Guards §6.3 + §3.1 `match` semantics + Failure mode #2 honesty. kind: test. estimate_minutes: 35. Files: `scripts/__tests__/decision-rollup-handsoff.xfail.bats` (new), `scripts/__tests__/fixtures/decisions/handsoff-10/log/*.md` (new — 10). DoD: tests xfail until T4 lands the rollup pass; assertions pinned to §3.2 schema rule "hands-off autodecides count separately". Committed before T4 per Rule 12.

- [ ] **TT4-axisgate** — xfail bats for the **axis-introduction gate invariant** (existing §Test plan bullet, currently undefended). Cases: (a) fixture log file tags `axes: [scope-vs-debt, undeclared-axis]`; `axes.md` contains only `scope-vs-debt`; running `regenerate_decisions_index` exits non-zero with stderr `[lib-decision] BLOCK: undeclared axis "undeclared-axis"` and does not write a partial INDEX; (b) deprecated axis (`deprecated: 2026-05-01` in `axes.md`) tagged on a NEW log file (mtime after deprecation date) is also rejected; (c) historical log already tagging the now-deprecated axis is preserved (§3.4 retention) — rollup passes with the deprecated axis still appearing in `preferences.md` with a `(deprecated)` marker. Guards §3.4 axis retention + §4.3 strictness. kind: test. estimate_minutes: 40. Files: `scripts/__tests__/decision-axis-gate.xfail.bats` (new), `scripts/__tests__/fixtures/decisions/axis-gate/**` (new). DoD: tests xfail until T4 lands; deprecation logic exercised by an `axes.md` fixture. Committed before T4 per Rule 12.

- [ ] **TT5-eager** — xfail bats for the **eager-boundary invariant** (existing §Test plan bullet). Asserts: (a) the rendered initialPrompt of `.claude/agents/evelynn.md` contains an instruction to read `agents/evelynn/memory/decisions/preferences.md` AND `agents/evelynn/memory/decisions/axes.md` at boot positions 8 and 9 per §6.1; (b) the same initialPrompt does NOT contain any string matching `decisions/INDEX\.md` or `decisions/log/` (negative grep — boot must not eager-load the lazy surfaces); (c) same two assertions for `.claude/agents/sona.md`; (d) `agents/evelynn/CLAUDE.md` and `agents/sona/CLAUDE.md` Startup Sequence sections contain the new bullets per §6.4 with the same eager/lazy split. Guards §6.1 + §6.4 + §7.4 (NEVER eager-load the corpus). kind: test. estimate_minutes: 30. Files: `scripts/__tests__/decision-eager-boundary.xfail.bats` (new). DoD: text-level grep + negative grep; tests xfail until T9 + T10. Committed before T9 per Rule 12.

- [ ] **TT5-protocol** — xfail bats for the **capture-ritual shape invariant** (existing §Test plan bullet) + Operating Modes addendum. Cases: (a) `.claude/agents/evelynn.md` Decision Capture Protocol section literally contains `Predict:`, `Confidence:`, and the three-bucket enum `low|medium|high` (the *prediction-confidence* enum, three buckets — distinct from §3.5's four-bucket *display* enum); (b) the section references the `decision-capture` skill by name; (c) the Operating Modes addendum mentions both `coordinator_autodecided: true` and `hands-off autodecides counted separately`; (d) §6.2 step 3 ("retry once, then proceed without log") is present verbatim — guards Failure mode #5; (e) same five assertions for sona.md. Guards §6.2 + §6.3 + Failure mode #5. kind: test. estimate_minutes: 30. Files: `scripts/__tests__/decision-capture-protocol-shape.xfail.bats` (new). DoD: grep-only; xfail until T9. Committed before T9 per Rule 12.

- [ ] **TT-INT** — Cross-task **integration smoke** for the full coordinator-decision flow. Pipeline test in a temp git repo clone: (1) bootstrap `decisions/` tree from T8 fixtures (axes.md with 4 seeded axes, empty preferences.md template); (2) invoke `scripts/capture-decision.sh evelynn --file <fixture-log.md>` (T2) → file lands at `agents/evelynn/memory/decisions/log/<date>-<slug>.md`, exit 0, stdout = path; (3) run `scripts/memory-consolidate.sh evelynn --decisions-only` (T4) → INDEX.md regenerated with one row, preferences.md `Samples:` updated for tagged axes; (4) render the boot text of `.claude/agents/evelynn.md` (T9) and verify the new positions 8/9 instructions surface preferences.md content; (5) inject a second decision tagged the same axis with `match: false`, re-run rollup → `Notable misses:` line in preferences.md gains the new entry; (6) `/end-session` Step 6c shape grep (T6) confirms the regen happens before commit. One-shot exercises T2 + T4 + T6 + T9 + T10 + T8. Guards: the full coordinator-decision flow (write decision → INDEX update → coordinator-boot read → match-rate computation), §5.2 ordering, §9 boot order. kind: test. estimate_minutes: 60. Files: `scripts/__tests__/decision-pipeline-integration.xfail.bats` (new). DoD: smoke runs in <30s; tests xfail until T9 + T4 land. Committed before any task that gates the integration (T4) per Rule 12.

- [ ] **TT-INV** — **Invariant regression suite**. One bats file with one named test per invariant in the existing §Test plan: Schema, Rollup-idempotency, Ordering, Axis-introduction-gate, Hands-off-separation, No-orphan, Eager-boundary, Capture-ritual-shape (8 invariants → 8 tests), each with `# guards Invariant: <name>` comment. Examples: **No-orphan** — running `capture-decision.sh evelynn --file <log>` against a coordinator dir whose `decisions/log/` subdirectory does not exist exits non-zero with stderr `[capture-decision] BLOCK: log/ not bootstrapped — run T8`; **Ordering** — synthetic `/end-session` SKILL.md with Step 6c placed BEFORE Step 6b is detected by a shape-asserter and fails. Each test cites its invariant by name in a comment. Guards every named §Test plan invariant has at least one regression test (Rule 13 prophylaxis — these invariants are the regression surface). kind: test. estimate_minutes: 60. Files: `scripts/__tests__/decision-invariants.xfail.bats` (new). DoD: 8 named tests, each `# guards Invariant: <name>`; all xfail until the relevant impl task lands. Committed before T2 per Rule 12 (earliest impl whose code these tests will exercise).

### Schema-stability gaps (Xayah analysis)

Three concerns surfaced while authoring this test plan; each is now covered above but called out so future plan editors do not regress:

1. **The §3.5 mutation-simulation test is the load-bearing tripwire.** §3.5 is documentation-only without a test that proves a rename of `axes → topics` (or any of the four bind-points) actually trips the assertion. **TT1-bind** §(b)-§(c) implements four mutation cases — one per bind-point — with the same env-var-rename hook pattern that `feedback-index-bind-contract.xfail.bats` uses (commit `9a3a9257`). If TT1-bind is dropped or its mutation cases descoped, the dashboard plan's Phase 1 has no upstream tripwire and the §3.5 bind contract becomes aspirational. **Recommend Rakan implements TT1-bind first**, before any other test task in this plan.

2. **Confidence enums are split across two surfaces and must not collide.** The §3.5 *display* enum (`low|medium|medium-high|high`, four buckets, derived from sample count per §3.2) and the §6.2 *prediction* enum (`low|medium|high`, three buckets, subjective at decision time) are different. TT5-protocol §(a) and TT1-bind §(d) test them independently. If a future schema edit collapses them into a shared enum, both tests trip — that is the desired behaviour. Flag for Aphelios if the impl phase reveals an unintended collision.

3. **The match-rate-honesty surface (§6.3 hands-off separation) has no upstream regression test today.** The existing §Test plan bullet "Hands-off separation invariant" cited the bucket-counting requirement but landed no fixture. **TT4-handsoff** closes this gap with a 10-file fixture (3 hands-off + 7 explicit) and asserts the `+3 hands-off` parenthetical column appears in `Samples:` per §3.2. Without this fixture, the v2 auto-decide gating plan (§10.5) inherits a corpus with potentially-inflated match-rates.

### Open questions for Swain (complex-track adjudication)

- **OQ-T1** — TT1-bind's env-var-rename hook is a test-only affordance (`DECISION_RENAME_AXES`, etc.). Should the validator implement these env hooks as part of the production `_lib_decision_capture.sh` (cleaner — tests use the same code path) or as a test-side stub that monkey-patches the validator (smaller production surface, but a test-prod skew risk)? Recommend **production-side hooks gated behind `DECISION_TEST_MODE=1`**, mirroring the feedback-system's `FEEDBACK_INDEX_RENAME_*` pattern at commit `9a3a9257`. Decision needed before T2 implementation.
...
- **OQ1** — Should the initial seed of `axes.md` (T8) include 4 axes or a smaller kernel (e.g., just `scope-vs-debt`)? Recommendation: **4 seeded axes**. Fewer seeds force the coordinator into "propose new axis" mode in the first week, which is noisy. Four concrete starters (scope-vs-debt, explicit-vs-implicit, hand-curated-vs-automated, rollout-phased-vs-single-cutover) cover ~60% of decisions observed in the current `agents/memory/duong.md` + recent shards. Surface to Duong if he wants a different opening set.
  **Pick:** 4 seeded axes as recommended — proceed with `scope-vs-debt`, `explicit-vs-implicit`, `hand-curated-vs-automated`, `rollout-phased-vs-single-cutover`.
- **OQ2** — Should `preferences.md` be boot-loaded before `axes.md` (current order) or after? Recommendation: **preferences before axes**, because the summary prose in `preferences.md` references axis names and the coordinator can still grep `axes.md` for exact definitions. Reversed order would make a cold read of `preferences.md` reference undefined-yet names. Flag if Duong finds the reversed order clearer on first pass.
  **Pick:** preferences before axes (current order) — proceed as specified in §9.
- **OQ3** — `Predict:` vs. `Pick:` divergence: should divergence be *allowed* (coordinator recommends `a` but privately predicts `b`)? Recommendation: **allowed**. The two serve different purposes: `Pick:` is the coordinator's independent judgement of the best answer; `Predict:` is the forecast of Duong's answer. When they diverge, the coordinator is implicitly saying "I think option X is correct but I expect you to choose Y based on our history" — that is a valuable piece of information for Duong to see. An enforcement rule that `Predict == Pick` collapses the two into one, losing that signal.
  **Pick:** Divergence allowed — `Predict:` and `Pick:` are independent fields; no enforcement constraint between them.
