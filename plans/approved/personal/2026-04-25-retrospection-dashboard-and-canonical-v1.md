...
- [x] **T.COORD.3** Author `architecture/canonical-v1.md` lock manifest. estimate_minutes: 60. kind: coord-arch. parallel_slice_candidate: no. Detail: hand-curated one-pager per §Q6. Lists agent-def SHAs at lock time, `agents/memory/agent-network.md` SHA, `CLAUDE.md` rule-1-21 SHAs, `.claude/settings.json` SHAs, lock-tag git ref `canonical-v1`. Owner: Evelynn (coordinator authorship — not a builder task). Files: `architecture/canonical-v1.md`. DoD: file exists; lists every `.claude/agents/*.md` SHA at lock-tag time; references `architecture/canonical-v1-bypasses.md`; one-page (≤200 lines); **includes a simple manual disable mechanism** (e.g. an env var `CANONICAL_V1_LOCK=off` or a single-line marker file `.canonical-v1-unlocked` honored by any future enforcement scripts) — Duong directive 2026-04-27: system will iterate frequently in early lifetime; the lock must be cheap to disable for a planned amendment session and re-enable when that session closes. Disable mechanism documented inline with a one-paragraph "when to use" note.
- [x] **T.COORD.4** Author `/canonical-retro` skill scaffold. estimate_minutes: 45. kind: coord-skill. parallel_slice_candidate: no. Detail: skill body that Evelynn invokes Saturday 09:00 Asia/Bangkok per §Q6. Dispatches Lux + Karma + Swain over the past week's `events.jsonl` slice + `feedback/INDEX.md` + `architecture/canonical-v1-bypasses.md`; output is `plans/proposed/personal/YYYY-MM-DD-canonical-v2-rationale.md` authored by Swain. Owner: Evelynn (skill author). Files: `.claude/skills/canonical-retro/SKILL.md`. DoD: skill renders the dispatch chain; produces a Swain-authored ADR template; ≤150 lines.
...
### Per-task test specs (T.P2.1–T.P2.5) — Phase 2 expansion (Xayah 2026-04-26 complex track)

Phase 2 testing centers on **three independent SQL+source-reader pairs** (feedback-rollup, decision-rollup, coordinator-weekly) that converge into a coordinator-detail HTML page. The parallel-yes window between T.P2.2/T.P2.3/T.P2.4 means three xfail test commits land on a shared base before three impl commits land in disjoint worktrees — every xfail must be self-contained (no cross-rollup imports) so the three impls can flip the three xfails independently. The home-page integration (T.P2.5) is paired-commit (xfail snapshot, then template wiring) per Rule 12. Total Phase-2 test surface: **8 xfail tasks, 320 minutes.**

#### Level 1 — Unit / fixture-level (Phase 2)

- [ ] **TP2.T1** — xfail unit test: feedback-rollup query against golden fixture (xfail). estimate_minutes: 35. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/queries-feedback-rollup.test.mjs`, `tools/retro/fixtures/feedback-index.md`, `tools/retro/queries/feedback-rollup.expected.json`. Kind: unit. DoD: (a) hand-curated `feedback-index.md` fixture carries six rows spanning all three severities (`high/medium/low`), all three statuses (`open/triaged/closed`), and at least two distinct `category` values per plan A §D12's read contract; (b) test reads the fixture, runs the (eventually-implemented) `feedback-rollup.sql` via DuckDB CLI subprocess against an `events.jsonl` derived from the fixture, deep-equal against `feedback-rollup.expected.json`; (c) golden enforces shape `(category, severity, status, open_count, latest_entry_ts)` exactly — extra columns fail the test (Phase-2-boundary check, prevents schema drift); (d) timestamp normalization: `latest_entry_ts` is the entry's `created` frontmatter, NOT file mtime (deterministic across re-runs, immune to git-clone timestamp loss); (e) negative-coverage row: a `status: closed` entry with `open_count > 0` MUST fail the assertion (prevents the rollup from counting closed entries — guards plan A §D12 contract); (f) suite is xfail (skip if `tools/retro/queries/feedback-rollup.sql` absent) with `Plan-Ref:` trailer citing this ADR; (g) commit prefix `chore:`. Guards: T.P2.2. Committed before T.P2.2 per Rule 12.

- [ ] **TP2.T2** — xfail unit test: decision-rollup query + axes parser invariants (xfail). estimate_minutes: 40. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/queries-decision-rollup.test.mjs`, `tools/retro/__tests__/decision-axes-parser.test.mjs`, `tools/retro/fixtures/decisions/evelynn/2026-04-NN-<slug>.md` ×3, `tools/retro/queries/decision-rollup.expected.json`. Kind: unit. DoD: (a) three decision fixtures: one with `match: true` + `coordinator_confidence: high`, one with `match: false` + `coordinator_confidence: medium`, one with `duong_concurred_silently: true` (no explicit `match` field — parser derives `match: true` per plan B §3.1 line 136); (b) parser unit asserts confidence mapping `low/medium/high → 1/2/3` is total (any other value raises a typed error, NOT a silent zero); (c) parser asserts `axes` field is a non-empty array of strings — empty/missing fails ingest with a diagnostic (data-quality guard); (d) `decision-rollup.sql` golden returns `(coordinator, axis, decisions_total, decisions_matched, match_rate, avg_confidence_at_time)` with `match_rate = matched/total` to 4-decimal precision (string-compare not float-compare); (e) **negative invariant**: a fixture file containing `axes: [routing-track]` and a sibling containing `axes: [routing-track, model-tier]` produces TWO rollup rows for `routing-track` (axis-explosion, NOT axis-array dedup) — guards against the bug-class where `axes` is treated as a set per coordinator instead of per decision; (f) **silent-concur edge**: re-running the parser over the `duong_concurred_silently: true` fixture twice produces byte-identical event records (no `Date.now()` leak); (g) xfail until T.P2.3 lands. Guards: T.P2.3. Committed before T.P2.3 per Rule 12.

- [ ] **TP2.T3** — xfail unit test: coordinator-weekly full + prompt-stats invariants (xfail). estimate_minutes: 50. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/queries-coordinator-weekly.test.mjs`, `tools/retro/__tests__/prompt-stats.test.mjs`, `tools/retro/fixtures/parent-session-with-prompts.jsonl`, `tools/retro/queries/coordinator-weekly.expected.json`. Kind: unit. DoD: (a) parent-session fixture contains exactly three subagent dispatches whose first user-message prompts hit known percentile points: dispatch A (200 chars, 0 headers, 0 plan-citations, 0 concern-tags), dispatch B (1200 chars, 4 headers, 1 plan-citation, 1 concern-tag), dispatch C (3500 chars, 8 headers, 2 plan-citations, 1 concern-tag); (b) prompt-stats unit asserts `prompt_chars_p50 = 1200`, `prompt_chars_p95 = 3500` (interpolation rule documented in lib header); (c) `concern_tag_present_pct = 66.66` (2 of 3) and `plan_citation_present_pct = 66.66` — pct values are 4-decimal strings to defeat float drift; (d) compression ratio: dispatch B emits 800 subagent output tokens against 300 prompt tokens → ratio `2.6667`; assert per-dispatch ratio is computed and the weekly p50/p95 are derived from the per-dispatch distribution (not from an aggregate sum); (e) **inline-vs-delegate ratio invariant** (§Q8): given a fixture with 4 inline tool-uses on the parent JSONL and 12 sidechain tool-uses across child subagents, assert `delegate_ratio = 12/(4+12) = 0.7500` (string-compare); flag = `healthy` (>0.7); fixture-flip variants: `0.6500 → drift`, `0.4000 → executor-mode` per §Q8 thresholds; (f) **regex-pin invariant** (T.P2.4 DoD-(e)): assert plan-citation regex `plans/(proposed|approved|in-progress|implemented|archived)/(personal|work)/.+\.md` matches `plans/in-progress/personal/2026-04-25-foo.md` and rejects `plans/foo.md` (no stage subdir) and `plans/proposed/foo.md` (no concern subdir) — guards against silent regex regression on plan-tree shape; (g) golden `coordinator-weekly.expected.json` carries all twelve columns from T.P2.1 DoD-(e); (h) xfail until T.P2.4 lands. Guards: T.P2.4. Committed before T.P2.4 per Rule 12.

- [ ] **TP2.T4** — xfail invariant test: ingest source-extension paths are mtime-cache safe (xfail). estimate_minutes: 30. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/invariant-source-extension-mtime.test.mjs`, `tools/retro/fixtures/mtime-cache-scenarios/`. Kind: unit. DoD: (a) three sub-fixtures replay an ingest sequence: scenario-1 cold start (no `events.mtimecache`); scenario-2 warm start (cache present, no source changes — assert ZERO new events emitted); scenario-3 partial-update (only `feedback/INDEX.md` modified — assert events emitted ONLY for the feedback-index source, NOT for parent-session/subagent/sentinel/decision-log sources); (b) invariant: a corrupted `events.mtimecache` (truncated JSON) MUST trigger a full re-scan with a logged warning, NOT a silent partial-emission (data-loss guard); (c) invariant: re-running ingest twice over the same source set with no source-mtime changes produces byte-identical `events.jsonl` AND byte-identical `events.mtimecache` (idempotence under no-op); (d) covers all FIVE source readers seeded by Phase 2: parent-jsonl, subagent-jsonl, sentinel, feedback-index, decision-log; (e) xfail (skipped via `{ skip: !existsSync(ingestPath) }`) until at least one of T.P2.2/T.P2.3 lands so `lib/sources.mjs` carries the new readers; (f) **cross-task guard**: this test fails if T.P2.2 ships its feedback-index reader without wiring the per-source mtime tracking from T.P1.2 lib/mtime-cache.mjs — catches the bug-class where a parallel-track impl forgets the cache integration. Guards: T.P2.2 + T.P2.3 + T.P2.4. Committed before any of T.P2.2/3/4 per Rule 12 (lands on the post-T.P2.1 base SHA — see §"Phase 2 fan-out commit ordering" below).

#### Level 2 — Integration (Phase 2)

- [ ] **TP2.T5** — xfail integration test: end-to-end Phase-2 ingest+render against full fixture corpus (xfail). estimate_minutes: 35. parallel_slice_candidate: no. Files: `tools/retro/__tests__/e2e-phase2.bats`, `tools/retro/fixtures/e2e-phase2/`. Kind: integration (bats). DoD: (a) seeds a fixture HOME containing the Phase-1 corpus PLUS `feedback/INDEX.md` PLUS three decision logs PLUS the prompt-stats parent-session; (b) runs `npm run retro:ingest && npm run retro:render`; (c) asserts `dist/data/feedback-rollup.json`, `dist/data/decision-rollup.json`, `dist/data/coordinator-weekly.json` all exist and deep-equal their respective `.expected.json` files; (d) asserts `dist/coordinator-evelynn-week-<iso>.html` carries the inline-vs-delegate ratio panel, three histogram canvases (prompt length, header count, compression ratio), and the per-axis match-rate table; (e) asserts `dist/index.html` carries the feedback open-entries tile (severity-grouped) AND the lowest-match-rate axes tile per §Q4 home-page extensions; (f) **regression invariant**: asserts the Phase-1 `plan-<slug>.html` pages emitted by T.P1.6 are byte-identical to their Phase-1 baseline (Phase-2 must not perturb Phase-1 output); (g) total wall time <8s; (h) xfail until T.P2.5 lands. Guards: T.P2.5 acceptance gate. Committed before T.P2.5 per Rule 12.

- [ ] **TP2.T6** — xfail snapshot test: coordinator-detail + extended index HTML (xfail). estimate_minutes: 35. parallel_slice_candidate: no. Files: `tools/retro/__tests__/render-html-phase2.test.mjs`, `tools/retro/__tests__/__snapshots__/coordinator-detail-evelynn-week-NN.html.snap`, `tools/retro/__tests__/__snapshots__/index-with-phase2-tiles.html.snap`. Kind: unit (snapshot). DoD: (a) coordinator-detail snapshot asserts every `(axis, decisions_total, decisions_matched, match_rate)` row from `decision-rollup.expected.json` appears in the per-axis match-rate `<table>` with stable column order; (b) snapshot asserts inline-vs-delegate ratio panel renders the `flag` value (`healthy/drift/executor-mode`) with class names `flag-healthy/flag-drift/flag-executor-mode` for CSS targeting; (c) histogram canvases asserted as inline `<svg>` with `<rect>` children — assert `width` attribute is integer-valued (no float pixel rounding non-determinism); (d) **R2 determinism guard carried forward**: regex scan rejects `Date.now()`, `Math.random()`, `process.pid`, `new Date(` substrings; assert two consecutive renders of the same fixture produce byte-identical HTML; (e) **§Q4 SPA-rejection guard carried forward**: `vue|pinia|vue-router` substring scan returns zero hits across both snapshots; exactly one inline `<script>` block per page; (f) home-page extended snapshot asserts the feedback-tile severity counts match `feedback-rollup.expected.json` aggregation (high/medium/low buckets with deep-link `href="../feedback/INDEX.md"` per §Q4); (g) home-page extended snapshot asserts the lowest-match-rate-axes tile shows the THREE axes with lowest match-rate across both coordinators, sample-size displayed (n=N) per axis; (h) xfail until T.P2.5 impl-side lands. Guards: T.P2.5 (snapshot half of the paired Rule-12 commit). Committed as the test-half of T.P2.5's paired commit per Rule 12.

#### Phase 2 fan-out commit ordering

The Phase-2 fan-out has a Rule-12 subtlety the breakdown does not call out: T.P2.2/T.P2.3/T.P2.4 are dispatched in **three concurrent worktrees** off the post-T.P2.1 commit SHA. Each impl worktree must carry its own xfail commit ahead of its impl commit. Recommended order on the shared base:

1. T.P2.1 fixture/expected-event-log commit (already breakdown-scheduled).
2. **TP2.T1, TP2.T2, TP2.T3, TP2.T4** xfail commits land on the shared base (single worktree, four serial chore commits — total ~155 min). This puts every Phase-2 xfail on the base SHA before fan-out.
3. Fan-out — three worktrees take T.P2.2/T.P2.3/T.P2.4 in parallel, each flipping its respective xfail green in its impl commit.
4. Convergence — T.P2.5 paired commits (TP2.T6 xfail + impl) plus TP2.T5 e2e (xfail before T.P2.5 impl).

This satisfies Rule 12 on every branch tip without serializing the impl tier.

### Per-task test specs (T.P3.1–T.P3.4) — Phase 3 expansion (Xayah 2026-04-26 complex track)

Phase 3 introduces the **canonical-v1 manifest lock-bypass scanner** (T.P3.1, the core invariant surface for §D8 / Rule-19 alignment), the home-page lock-tile + stale-banner rendering (T.P3.2), the gated optional Lux grading pass (T.P3.3), and the e2e bats acceptance smoke (T.P3.4). T.COORD.3 (`canonical-v1.md` lock manifest authoring) hard-gates T.P3.1 — the scanner enumerates manifest paths and cannot run without the manifest existing. The cross-plan dependency T.COORD.3 → T.P3.1 is exercised explicitly in TP3.T1's setup. Total Phase-3 test surface: **6 xfail tasks, 270 minutes.**

#### Canonical-v1 invariants (the §D8 surface)

The §D8 invariants the lock-bypass scanner must protect (per §Q6 lock manifest semantics):

- **I-LOCK-1**: every commit during the lock-week whose touched-file set intersects the manifest path-set MUST carry a `Lock-Bypass: <reason>` trailer.
- **I-LOCK-2**: a `Lock-Bypass:` trailer present on a commit whose touched-file set is DISJOINT from the manifest path-set is itself a violation (false-positive trailer = noise; prevents the trailer from being slapped on every commit defensively).
- **I-LOCK-3**: every `Lock-Bypass:` trailer commit MUST have a corresponding row in `architecture/canonical-v1-bypasses.md`. Trailer-without-log-row and log-row-without-trailer are both violations.
- **I-LOCK-4**: bypass severity MUST match between the trailer (if specified) and the log row; mismatch = violation.
- **I-LOCK-5**: outside the lock-week (commits before the lock-tag or after the v2-rationale ADR ships), the scanner emits zero `kind: lock-violation` events even on manifest-path edits — lock applies only to the active lock-week.
- **I-LOCK-6**: `--no-verify` on a manifest-path-touching commit during the lock-week is a hard violation regardless of trailer presence (per Rule 14 + §Q6).

#### Level 1 — Unit / fixture-level (Phase 3)

- [ ] **TP3.T1** — xfail unit test: lock-bypass scanner I-LOCK-1 / I-LOCK-2 / I-LOCK-5 invariants (xfail). estimate_minutes: 55. parallel_slice_candidate: no. Files: `tools/retro/__tests__/lock-bypass-scanner.test.mjs`, `tools/retro/fixtures/canonical-v1-manifest.md`, `tools/retro/fixtures/git-log-with-bypass.json`, `tools/retro/fixtures/git-log-clean-week.json`, `tools/retro/fixtures/git-log-pre-lock.json`. Kind: unit. DoD: (a) `canonical-v1-manifest.md` fixture enumerates 8 paths spanning all four lock-set categories — agent defs, agents-network, CLAUDE.md, settings.json — sufficient for path-intersection coverage; (b) **I-LOCK-1 sub-test**: git-log fixture carries a manifest-path-touching commit with `Lock-Bypass: typo fix in agent description, severity: low` trailer → assert `kind: lock-bypass` event emitted with `severity: low`; (c) **I-LOCK-1 violation sub-test**: same fixture with the trailer removed → assert `kind: lock-violation` event emitted with diagnostic `missing Lock-Bypass trailer for manifest-path edit`; (d) **I-LOCK-2 sub-test**: git-log fixture carries a NON-manifest-path commit (`apps/web/foo.ts`) with `Lock-Bypass: spurious trailer` → assert `kind: lock-violation` event with diagnostic `Lock-Bypass trailer on non-manifest commit`; (e) **I-LOCK-5 sub-test**: pre-lock-tag git-log fixture (commits before the lock-tag commit date) with manifest-path edits → assert ZERO violation events emitted regardless of trailer presence (lock only applies in-week); (f) **scanner determinism**: re-running the scanner over the same fixture corpus produces byte-identical `events.jsonl` (sorted by commit-author-date, ties broken by SHA — documented in lib header); (g) **manifest-missing failure mode**: with `architecture/canonical-v1.md` absent (T.COORD.3 not yet shipped), scanner exits non-zero with diagnostic `lock manifest missing — run T.COORD.3 first` — exercises the cross-plan dependency T.COORD.3 → T.P3.1; (h) xfail (skipped via `{ skip: !existsSync('tools/retro/lib/lock-bypass.mjs') }`) until T.P3.1 lands; (i) **cross-plan dependency exercise**: a sub-test stubs `architecture/canonical-v1.md` with the fixture content, runs the scanner end-to-end, and asserts the manifest-path-set is loaded from the fixture rather than hard-coded — guards against the impl drifting away from the manifest. Guards: T.P3.1. Committed before T.P3.1 impl per Rule 12.

- [ ] **TP3.T2** — xfail unit test: bypass-log reconciliation I-LOCK-3 / I-LOCK-4 / I-LOCK-6 invariants (xfail). estimate_minutes: 40. parallel_slice_candidate: no. Files: `tools/retro/__tests__/lock-bypass-reconcile.test.mjs`, `tools/retro/fixtures/canonical-v1-bypasses-clean.md`, `tools/retro/fixtures/canonical-v1-bypasses-orphan-row.md`, `tools/retro/fixtures/canonical-v1-bypasses-severity-mismatch.md`, `tools/retro/fixtures/git-log-no-verify.json`. Kind: unit. DoD: (a) **I-LOCK-3 sub-test (trailer-no-row)**: git-log carries a `Lock-Bypass:` trailer commit but the bypass-log fixture has no matching `sha` row → assert `kind: lock-violation` event with diagnostic `bypass trailer not logged`; (b) **I-LOCK-3 sub-test (row-no-trailer)**: bypass-log fixture has an entry whose `sha` does not appear in any `Lock-Bypass:`-trailer commit → assert `kind: lock-violation` with diagnostic `bypass log row has no matching trailer commit`; (c) **I-LOCK-4 sub-test**: trailer says `severity: low`, log row says `severity: high` → assert violation event; severity-promotion (low→high) and severity-demotion (high→low) are BOTH violations (no asymmetry); (d) **I-LOCK-6 sub-test**: git-log fixture carries a manifest-path-touching commit whose `--no-verify` flag is detectable via the absence of pre-commit hook fingerprint (heuristic: commit message lacks the auto-injected hook-pass marker, OR ref-log shows direct-commit-on-protected-branch); document the heuristic limit — if `--no-verify` is undetectable post-hoc the violation flips to `kind: lock-violation-suspected` instead of hard `kind: lock-violation` (truthful test for what's actually observable from git history alone); (e) **clean-week control**: a fixture with three valid trailer/log pairs and zero orphans emits zero violation events (negative control); (f) **rollup query test**: `lock-bypass-rollup.sql` against the violation event set returns one row per `(iso_week, severity)` with `count` and `reconciled` boolean; `reconciled` flips true when the ADR-fixture for the next-Saturday retro mentions the bypass `sha`; (g) xfail until T.P3.1 lands. Guards: T.P3.1 (rollup half). Committed before T.P3.1 impl per Rule 12.

- [ ] **TP3.T3** — xfail snapshot test: home-page lock tile + stale banner rendering states (xfail). estimate_minutes: 45. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/render-lock-tile.test.mjs`, `tools/retro/__tests__/__snapshots__/index-lock-tile-fresh.html.snap`, `tools/retro/__tests__/__snapshots__/index-lock-tile-stale.html.snap`, `tools/retro/__tests__/__snapshots__/index-lock-tile-no-bypass.html.snap`, `tools/retro/__tests__/__snapshots__/index-lock-week-active.html.snap`, `tools/retro/fixtures/canonical-v2-rationale-fresh.md` (3-day-old retro fixture), `tools/retro/fixtures/canonical-v2-rationale-stale.md` (16-day-old), `tools/retro/fixtures/canonical-v2-rationale-edge-14d.md` (exactly-14-day boundary). Kind: unit (snapshot). DoD: (a) **fresh-state snapshot**: with a 3-day-old retro fixture present and 2 in-week bypasses (1 high, 1 low), assert lock-tile renders `Active lock: canonical-v1`, `days since last retro: 3`, `bypasses this week: 2 (1 high, 1 low)`, NO stale banner; (b) **stale-state snapshot**: with a 16-day-old retro fixture, assert stale banner present with class `lock-banner-stale` and copy `Weekend retro overdue (16 days)`; (c) **boundary snapshot**: with exactly-14-day fixture, assert stale banner ABSENT (boundary inclusive — 14 days is not yet stale, >14 is stale per breakdown DoD-(b) wording); 15-day fixture variant asserts banner PRESENT (boundary exclusive on the high side); (d) **no-bypass snapshot**: with a fresh retro and zero in-week bypasses, assert tile renders `bypasses this week: 0` with class `lock-bypass-clean`; (e) **lock-week-active badge state**: snapshot variant with current date inside the lock-week (Mon-Sun span computed from lock-tag commit-date), assert badge appears with class `lock-week-active`; outside-lock-week variant asserts badge absent; (f) **R2 determinism guard**: regex scan rejects `Date.now()` substring leaks; assert fixed-clock injection point exists in `html-generator.mjs` (`lib/html-generator.mjs` accepts a `now` parameter for testability — guards against time-based test flakiness); (g) **§Q4 SPA-rejection guard carried forward**; (h) xfail until T.P3.2 lands. Guards: T.P3.2. Committed before T.P3.2 impl per Rule 12. Parallel-able with TP3.T4 (disjoint files; both share `index.html.tpl` snapshot but T.P3.3 emits its grade tile to a separate page section keyed by env flag — see breakdown DoD-(d)).

- [ ] **TP3.T4** — xfail unit test: optional Lux grading pass `RETRO_QUALITY_GRADE` gate behavior (xfail). estimate_minutes: 50. parallel_slice_candidate: yes. Files: `tools/retro/__tests__/quality-grader-gate.test.mjs`, `tools/retro/__tests__/quality-grader-cost-ceiling.test.mjs`, `tools/retro/fixtures/anthropic-graded.json` (mocked Anthropic record-replay payload), `tools/retro/fixtures/quality-grader-overspend-tokens.json`. Kind: unit. DoD: (a) **gate-off invariant**: with `RETRO_QUALITY_GRADE` unset OR set to any non-`1` value (`""`, `"0"`, `"true"`, `"yes"`), assert ingest emits ZERO `kind: quality-grade` events and `quality-grade-rollup.json` is empty `[]` — explicit equality to literal `'1'` is the only enable path (defends against truthy-but-not-`1` false enables); (b) **gate-on dry-run**: with `RETRO_QUALITY_GRADE=1` AND a small fixture (10 dispatches), assert dry-run token-count phase runs BEFORE any Anthropic call and computes estimated weekly spend; assert estimated cost ≤ $1 budget (per breakdown DoD-(b)); (c) **gate-on record-replay**: with `RETRO_QUALITY_GRADE=1` and `mocked-anthropic-endpoint` env override, assert grading uses the record-replay fixture (NO live API call — test fails if an outbound request is attempted via a request-spy); (d) **cost-ceiling kill-switch invariant**: with a fixture that forces the dry-run token estimator to project >$5 weekly spend (10000 dispatches at known per-dispatch token count), assert `quality-grader.mjs` aborts with non-zero exit and diagnostic `cost ceiling exceeded — refusing to grade`, NO Anthropic call attempted; (e) **lazy-import invariant**: with `RETRO_QUALITY_GRADE` unset, assert `@anthropic-ai/sdk` is NOT loaded — verified by intercepting `import()` and asserting the SDK module path is never resolved (defends against accidental top-level static imports); (f) **rollup golden under gate-off**: `quality-grade-rollup.expected.json` under default gate is the empty array `[]`; under gate-on the golden contains 4 buckets `clear/acceptable/wandering/under-spec'd` with deterministic counts derived from the record-replay fixture; (g) xfail until T.P3.3 lands. Guards: T.P3.3. Committed before T.P3.3 impl per Rule 12. Parallel-able with TP3.T3 — disjoint files except README.md ordering (trivial merge).

#### Level 2 — Integration (Phase 3)

- [ ] **TP3.T5** — xfail integration test: cross-plan dependency T.COORD.3 → T.P3.1 enforcement (xfail). estimate_minutes: 30. parallel_slice_candidate: no. Files: `tools/retro/__tests__/cross-plan-coord3-p31.bats`, `tools/retro/fixtures/coord3-fixtures/`. Kind: integration (bats). DoD: (a) **dispatch-order test**: setup phase removes `architecture/canonical-v1.md`; runs `npm run retro:ingest`; asserts ingest exits non-zero with the T.COORD.3-prereq diagnostic from TP3.T1 DoD-(g); (b) **dispatch-order recovery test**: setup writes the fixture manifest at `architecture/canonical-v1.md`; re-runs ingest; asserts ingest exits zero AND `events.jsonl` carries the lock-bypass events that the manifest enumerates against; (c) **manifest-mutation test**: with the manifest already present, modify the fixture manifest to drop one path; re-run ingest; assert previously-flagged violations on that path are NO LONGER emitted (manifest is the runtime authority — guards against scanner caching the path-set across runs); (d) **coordinator-skill smoke**: optional sub-test (skipped if `.claude/skills/canonical-retro/SKILL.md` absent) asserts the T.COORD.4 skill reads `events.jsonl` + `feedback/INDEX.md` + `architecture/canonical-v1-bypasses.md` per §Q6 — pure file-presence/read-contract assertion, no skill execution; (e) total wall time <5s; (f) xfail until both T.COORD.3 and T.P3.1 ship. Guards: cross-plan T.COORD.3 ↔ T.P3.1 invariant. Committed on the T.P3.1 branch tip BEFORE T.P3.1 impl flips it green per Rule 12.

- [ ] **TP3.T6** — xfail e2e bats smoke: full Phase-3 acceptance (xfail). estimate_minutes: 50. parallel_slice_candidate: no. Files: `tools/retro/__tests__/e2e-phase3.bats` (extended), `tools/retro/fixtures/e2e-phase3/`, `tools/retro/__tests__/__snapshots__/index-phase3-acceptance.html.snap`, `tools/retro/fixtures/anthropic-graded.json` (shared with TP3.T4). Kind: integration (bats). DoD: (a) seeds a fixture HOME with the Phase-2 corpus PLUS `architecture/canonical-v1.md` PLUS a 3-day-old `canonical-v2-rationale.md` (within retro window, no stale banner) PLUS `architecture/canonical-v1-bypasses.md` with two clean reconciled bypass entries; (b) runs `npm run retro:ingest && npm run retro:render`; (c) asserts `dist/index.html` snapshot matches `index-phase3-acceptance.html.snap` (lock tile + lock-week-active badge + bypass count + NO stale banner + NO quality-grade tile under default-off gate); (d) re-runs with `RETRO_QUALITY_GRADE=1` against the mocked Anthropic record-replay fixture; asserts a quality-grade tile appears with the four buckets; (e) **regression invariant**: Phase-1 `plan-<slug>.html` and Phase-2 `coordinator-evelynn-week-<iso>.html` files are byte-identical to their previous-phase baselines (Phase-3 must not perturb earlier phases); (f) **lock-violation surface invariant**: an extra fixture variant injects a `Lock-Bypass:`-missing manifest-path-touching commit; asserts `dist/index.html` lock-tile renders the violation count with class `lock-violation-present` and a deep-link to `architecture/canonical-v1-bypasses.md`; (g) total wall time <10s; (h) xfail until T.P3.4 lands. Guards: T.P3.4 acceptance gate + Phase-3 regression. Committed before T.P3.4 impl per Rule 12.

### Coverage matrix (Phase 2 + Phase 3)

| Test task | Kind | Guards impl task(s) | Rule 12 commit-before |
|---|---|---|---|
| TP2.T1 | unit | T.P2.2 | T.P2.2 |
| TP2.T2 | unit | T.P2.3 | T.P2.3 |
| TP2.T3 | unit | T.P2.4 | T.P2.4 |
| TP2.T4 | unit | T.P2.2 + T.P2.3 + T.P2.4 (cross-task mtime invariant) | first of T.P2.2/3/4 |
| TP2.T5 | integration | T.P2.5 | T.P2.5 |
| TP2.T6 | unit (snapshot) | T.P2.5 (paired) | T.P2.5 |
| TP3.T1 | unit | T.P3.1 | T.P3.1 |
| TP3.T2 | unit | T.P3.1 | T.P3.1 |
| TP3.T3 | unit (snapshot) | T.P3.2 | T.P3.2 |
| TP3.T4 | unit | T.P3.3 | T.P3.3 |
| TP3.T5 | integration | T.COORD.3 ↔ T.P3.1 (cross-plan) | T.P3.1 |
| TP3.T6 | integration | T.P3.4 | T.P3.4 |

**Phase 2 + 3 totals: 12 xfail test tasks, 590 minutes** (added on top of Phase-1's 7 / 235 min for a grand-total **19 xfail test tasks, 825 minutes**).

### Slicing summary (Phase 2 + 3)

| Test task | parallel_slice_candidate | Rationale |
|---|---|---|
| TP2.T1 | yes | Disjoint files (feedback-rollup) from TP2.T2/T3; >30 min; low merge friction. |
| TP2.T2 | yes | Disjoint files (decision-rollup); >30 min. |
| TP2.T3 | yes | Disjoint files (coordinator-weekly + prompt-stats); >30 min. |
| TP2.T4 | yes | Mtime-cache fixtures live in their own dir; cross-task assertion but file-disjoint from TP2.T1/2/3. |
| TP2.T5 | no | bats e2e — must follow T.P2.2/3/4 convergence; sequential by definition. |
| TP2.T6 | no | Paired-commit half of T.P2.5; sequential. |
| TP3.T1 | no | Foundational scanner test; T.P3.1 has no parallel siblings. |
| TP3.T2 | no | Same scanner surface as TP3.T1; same critical-path commit. |
| TP3.T3 | yes | Disjoint files from TP3.T4 (templates vs grader lib); >30 min; low merge friction (README ordering only). |
| TP3.T4 | yes | Disjoint files; gated-by-env path is fully isolated. |
| TP3.T5 | no | Cross-plan integration — must run on the converged T.P3.1 branch tip. |
| TP3.T6 | no | Final acceptance bats; sequential. |

**Parallel-yes count across Phase 2 + 3: 6 of 12 test tasks** (TP2.T1, TP2.T2, TP2.T3, TP2.T4, TP3.T3, TP3.T4). Phase 2 dispatches up to 4 xfail authors concurrently on the post-T.P2.1 base; Phase 3 dispatches up to 2 concurrently on the post-T.P3.1 base. Wait-bound: zero — every test task is bounded by author time, not external waits.

### Determinism + POSIX guarantees (Phase 2 + 3 carry-forward)

The Phase-1 guarantees from above carry forward unchanged. New Phase-2/3 additions:

- **Anthropic record-replay**: TP3.T4 + TP3.T6 both forbid live API calls via a request-spy; the record-replay fixture `anthropic-graded.json` is hand-curated, not auto-generated, so it doesn't drift across re-recordings.
- **Fixed-clock injection**: TP3.T3 DoD-(f) requires `html-generator.mjs` to accept a `now` parameter; this is the canonical defense against time-based flakiness in the stale-banner / lock-week-active rendering. Implementations passing `Date.now()` directly fail the snapshot-determinism guard.
- **Severity string normalization**: TP3.T2 I-LOCK-4 invariant uses string-equality on severity values; severity sets are pinned to lowercase `low|medium|high` per `architecture/canonical-v1-bypasses.md` schema header (T.P3.1 DoD-(e)).
- **Path-intersection determinism**: TP3.T1 fixture-manifest path-set drives the scanner's runtime authority — no hard-coded path-list in `lib/lock-bypass.mjs` (TP3.T5 DoD-(c) guards this).

### Gaps in Aphelios's breakdown (Xayah audit, 2026-04-26)

Items the breakdown omits that this test plan exposes — flagged for amendment, not blocking:

1. **T.P2.4 prompt-stats: per-dispatch event vs aggregate.** Breakdown DoD-(a) says "emitted as one event per dispatch with `kind: dispatch-prompt-stats`" — good. But the consumer query goldens in T.P2.1 imply weekly p50/p95 must be derived from the per-dispatch distribution (not from a session-level aggregate). TP2.T3 DoD-(d) makes this explicit. Recommend Aphelios add a one-liner to T.P2.4 DoD: "p50/p95 are aggregations OVER per-dispatch events, computed in SQL — not pre-aggregated in `prompt-stats.mjs`."

2. **T.P2.2/T.P2.3 mtime-cache integration.** Breakdown says T.P2.2 wires the new source into the per-source mtime cache; T.P2.3 does not have an analogous note. TP2.T4 cross-task invariant catches the omission, but it would be cleaner if T.P2.3 DoD added "(f) decision-log source registered with `lib/mtime-cache.mjs` per the T.P1.2 cache contract".

3. **T.P3.1 I-LOCK-2 (false-positive trailer) coverage.** Breakdown DoD calls out trailer-absent-violation (I-LOCK-1) but does not mention trailer-on-non-manifest-commit as a violation class (I-LOCK-2). TP3.T1 DoD-(d) covers it; recommend Aphelios update T.P3.1 DoD to add: "(g) `Lock-Bypass:` trailer on a NON-manifest-touching commit emits `kind: lock-violation` with diagnostic `spurious trailer` (prevents trailer-noise discipline drift)."

4. **T.P3.1 I-LOCK-6 (`--no-verify` detectability).** Breakdown silent on whether `--no-verify` violations are detectable from `git log` alone. TP3.T2 DoD-(d) introduces the `kind: lock-violation-suspected` heuristic. Recommend Aphelios document the heuristic boundary in T.P3.1 DoD or in `architecture/canonical-v1-bypasses.md` schema header.

5. **T.P3.2 stale-banner boundary semantics.** Breakdown says ">14 days triggers stale banner". Boundary handling on day-14 (banner present? absent?) is ambiguous. TP3.T3 DoD-(c) pins it to `>14 = stale, ≤14 = fresh` (banner absent at exactly 14, present at 15). Recommend Aphelios update T.P3.2 DoD to lock the boundary inclusive/exclusive choice.

6. **T.P3.3 truthy-but-not-`1` env values.** Breakdown says `RETRO_QUALITY_GRADE !== '1'` disables. TP3.T4 DoD-(a) extends the explicit equality contract to defeat `RETRO_QUALITY_GRADE=true` / `=yes` / `=on` accidental enables. Recommend Aphelios echo the strict-equality wording in T.P3.3 DoD-(a) for clarity.

7. **T.P3.4 lock-violation tile rendering.** Breakdown DoD-(c) lists what the home-page renders under default-off gate but does not specify the violation-present rendering state. TP3.T6 DoD-(f) covers it; recommend Aphelios add to T.P3.4 DoD: "(g) when lock-violation events are present, `dist/index.html` renders a `lock-violation-present` class on the lock-tile with a deep-link to `architecture/canonical-v1-bypasses.md`."

8. **Cross-plan dispatch ordering documentation.** Breakdown notes T.COORD.3 → T.P3.1 hard dependency in the Phase-3 dependency chain, but the failure mode (what the scanner does if T.COORD.3 hasn't shipped) isn't specified. TP3.T1 DoD-(g) + TP3.T5 DoD-(a) lock it: non-zero exit with a named diagnostic. Recommend Aphelios add this exit semantics to T.P3.1 DoD as the contracted failure mode.

None of these gaps blocks impl dispatch — the test plan covers each via the corresponding TP*.T* xfail. They are amendment candidates for the next breakdown-pass, not impl-pass blockers.
