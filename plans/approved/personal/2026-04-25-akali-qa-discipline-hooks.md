...
- **Rationale:** QA-plan structural checks pass (frontmatter `qa_plan: none` + justification, body §QA Plan present and consistent). Owner is karma; three tightly-scoped fixes (Lucian→Akali reminder, Akali chat-only refusal, verify-claims reminder) each have concrete files, DoDs, and a TDD test in T1 satisfying Rule 12. No TBDs or unresolved decisions in gating sections; the §Test plan ties each invariant to a specific test script. No-UI-surface QA-Waiver posture is consistent with `qa_plan: none`.
