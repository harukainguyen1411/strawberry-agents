...
orianna_signature_approved: "sha256:55ad245a40b354349e92606cb66a9cafa180f3e3a6d83768050ca653d253ca23:2026-04-22T06:17:11Z"
orianna_signature_in_progress: "sha256:55ad245a40b354349e92606cb66a9cafa180f3e3a6d83768050ca653d253ca23:2026-04-22T06:19:07Z"
---
...
2. **Signed-fix commit shape** — extend `scripts/hooks/pre-commit-orianna-signature-guard.sh`
   ...
   fix iteration.
3. **Stale git index lock auto-recovery** — at start of `scripts/orianna-sign.sh` <!-- orianna: ok -->
   and `scripts/plan-promote.sh`, detect a stale `.git/index.lock` <!-- orianna: ok --> file whose mtime is >60s old
   and which no live process holds (`lsof` empty); auto-clear with a loud
   audit line. Scope bounded to these two scripts.
4. **Batch-fix pre-pass** — new `scripts/orianna-pre-fix.sh` <!-- orianna: ok --> that applies the
   three known-safe mechanical rewrites against a plan (or set of plans)
   before the first Orianna invocation:
   - bare `tools/demo-studio-v3/<rest>` rewritten to the workspace-prefixed form <!-- orianna: ok -->
