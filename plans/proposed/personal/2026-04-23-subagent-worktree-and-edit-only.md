...
Caveat / audit note: the only legitimate "create new file" cases for these roles are (a) writing a learnings file at session end and (b) writing an inbox SendMessage. Both go through skills/scripts that use `Bash` (`tee`, redirected `cat`, or an inbox-send helper), not the `Write` tool. Removing `Write` does not block either path. T2 includes a quick grep audit to confirm no in-repo skill or doc instructs these agents to use `Write` directly.
...
- Files: `scripts/hooks/agent-default-isolation.sh` (new). <!-- orianna: ok -- prospective path, created by this plan -->
...
- Detail: hook reads JSON from stdin, extracts the subagent type field, locates the matching agent def file (e.g. `.claude/agents/aphelios.md`), parses the YAML frontmatter for `default_isolation: worktree`. If present AND the isolation tool-input field is unset/empty, emit a JSON response that mutates the isolation field to `"worktree"`. If the field is absent or already set, pass through (exit 0, no output). Must be POSIX-portable bash (rule 10): use `awk` or `sed` for YAML extraction, no `yq` dependency.
...
- Detail: in each file's frontmatter `tools:` list, remove the `- Write` line. Add a top-level `default_isolation: worktree` key. Quick grep audit on the four files (with ripgrep `Write`) should return only frontmatter/doc references that no longer instruct tool use. Also grep the skills tree for any "use the Write tool" guidance directed at these four to confirm no skill is broken.
...
- Files: `scripts/hooks/tests/test-agent-default-isolation.sh` (new). <!-- orianna: ok -- prospective path, created by this plan -->
- Files: `scripts/hooks/tests/test-edit-only-agents.sh` (new). <!-- orianna: ok -- prospective path, created by this plan -->
...
Wire the new tests into the pre-commit hook test runner (or whichever CI surface runs the hook-tests suite).

- Files: `scripts/hooks/tests/run-all.sh` if present, else just verify discovery glob already picks them up. <!-- orianna: ok -- conditional path; may or may not exist depending on current discovery mechanism -->
- Detail: confirm the new test scripts are picked up by whatever currently invokes the hook-tests suite. If discovery is glob-based, no edit needed — note that in the DoD.
...
INV-1 — **Auto-isolation fires for whitelisted subagent types.** When the coordinator invokes Aphelios, Kayn, Xayah, or Caitlyn via the Agent tool without an explicit `isolation` argument, the PreToolUse hook injects `isolation: "worktree"` before dispatch. Verified by the new isolation hook test (see T3 file path) feeding fake tool_input JSON for each whitelisted type and asserting the mutation. Negative case: invoking `yuumi` (no frontmatter field) leaves the isolation field unset.

INV-2 — **`Write` tool absent from the four agent defs.** The files `.claude/agents/aphelios.md`, `.claude/agents/kayn.md`, `.claude/agents/xayah.md`, `.claude/agents/caitlyn.md` do not list `Write` in their `tools:` block. Verified by the new edit-only agents test (see T3 file path) grepping each frontmatter.
