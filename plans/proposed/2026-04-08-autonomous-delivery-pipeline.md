...
- **Q5 — Billing → subscription plan only, never Claude API.** Duong explicit: "everything should run on the subscription plan, no API for claude." Agent work runs through Claude Code CLI against his Claude Max plan (single-account, shared usage quota across all logged-in devices, NOT seat-based), never via API keys. Dollar-cap circuit breaker is therefore not the right primitive — the constraint is a hard "no API calls" rule enforced at the agent-spawn layer. Detail phase must design a preflight check that blocks any agent that would call an Anthropic API endpoint.
...
- **Agent runtime → dual-mode local + GCE VM.** Duong clarified the subscription-vs-GCP tension directly: "I would normally have a machine running the CLI, but we also need a cloud option for auto. When I go to sleep and I close the machine or something like that, we need to have a cloud machine that can run the CLI, and of course it would be on Google." Concrete shape:
  ...
  - **Cloud mode (autonomous):** Always-on GCE VM on GCP, running Claude Code CLI, authenticated to Duong's Claude Max plan under `harukainguyen1411` (per the main-account directive). Primary for label-gated autonomous work. Takes over when the local box is asleep/closed/offline. Multi-device login with the same Max account is supported.
  - **No additional subscription cost** for the GCE VM — Max plan is single-account with usage-quota-shared-across-devices, not seat-based, so logging in the VM with the same Max account is free subscription-wise. Only cost is the GCE compute itself. Bottleneck is Max usage quota; if the pipeline burns it faster than expected, fix is behavioral (fewer parallel runs) or a plan upgrade, not another seat.
  ...
  - **Bootstrap:** `claude login` run once interactively on the GCE VM with Duong's Max-plan credentials (harukainguyen1411). Detailed phase must spec the exact provisioning flow.
