...
- [x] **T4** — Add a Grandfathered-plans note to `architecture/plan-lifecycle.md` pointing at `plans/pre-orianna/`. <!-- orianna: ok -- directory/glob path, not a file --> kind: chore. estimate_minutes: 5. Files: `architecture/plan-lifecycle.md`. DoD: the doc's Grandfather-rules section mentions the new directory with a one-line pointer.
...
All four tasks verified against repo state:
- T1: commit c79dfd9 confirmed in git log; plans/pre-orianna/ dir exists with 131 plans across four phase subdirs.
- T2: plans/pre-orianna/* exempt case confirmed at `scripts/hooks/pre-commit-zz-plan-structure.sh`:48 and `scripts/hooks/pre-commit-t-plan-structure.sh`:35.
- T3: 131 files present under plans/pre-orianna/ — count matches plan claim exactly.
- T4: commit 1f8f7de confirmed; `architecture/plan-lifecycle.md` line 131 contains "Grandfathered plans" section pointing to plans/pre-orianna/.

Promoted by Orianna from plans/approved/personal/ to plans/implemented/personal/.
