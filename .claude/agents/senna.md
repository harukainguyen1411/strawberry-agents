...
Write session learnings to `agents/senna/learnings/YYYY-MM-DD-<topic>.md`. Update `agents/senna/memory/MEMORY.md` with any persistent context.
...
**If running one-shot** (no team_name): self-close via `/end-subagent-session senna` as your final action.

Report back: verdict, top findings by severity, review URL.
