...
#   - busy_timeout = 5000  (ms; writer waits up to 5s before failing)
#   - synchronous = NORMAL (WAL-safe; faster than FULL; loses at most last tx on hard crash)
#
...
# ── §3: busy_timeout = 5000 ──────────────────────────────────────────────────
echo ""
echo "§3 PRAGMA busy_timeout"
# busy_timeout PRAGMA returns the current timeout in milliseconds.
BUSY_TIMEOUT=$(sqlite3 "$DB_PATH" "PRAGMA busy_timeout;")
if [ "$BUSY_TIMEOUT" -eq 5000 ]; then
  pass "busy_timeout = 5000 ms"
else
  fail "busy_timeout = '$BUSY_TIMEOUT' ms (want 5000)"
fi

# ── §4: synchronous = NORMAL (value 1) ───────────────────────────────────────
echo ""
echo "§4 PRAGMA synchronous"
# synchronous PRAGMA returns an integer: 0=OFF 1=NORMAL 2=FULL 3=EXTRA
SYNC_VAL=$(sqlite3 "$DB_PATH" "PRAGMA synchronous;")
if [ "$SYNC_VAL" -eq 1 ]; then
  pass "synchronous = NORMAL (1)"
else
  fail "synchronous = '$SYNC_VAL' (want 1 / NORMAL)"
fi

# ── §5: PRAGMAs survive a new connection opened via db_open ─────────────────
# journal_mode is persistent in the DB header once WAL is set; busy_timeout and
# synchronous are per-connection. This section asserts db_open sets them each time.
echo ""
echo "§5 PRAGMAs survive a fresh db_open call on the same DB"
...
BUSY2=$(sqlite3 "$DB_PATH2" "PRAGMA busy_timeout;")
SYNC2=$(sqlite3 "$DB_PATH2" "PRAGMA synchronous;")

if [ "$JOURNAL2" = "wal" ]; then
  pass "second connection: journal_mode = wal"
else
  fail "second connection: journal_mode = '$JOURNAL2' (want 'wal')"
fi

if [ "$BUSY2" -eq 5000 ]; then
  pass "second connection: busy_timeout = 5000 ms"
else
  fail "second connection: busy_timeout = '$BUSY2' ms (want 5000)"
fi

if [ "$SYNC2" -eq 1 ]; then
  pass "second connection: synchronous = NORMAL (1)"
else
  fail "second connection: synchronous = '$SYNC2' (want 1)"
fi
