...
<!-- #rule-plans-live-in-strawberry-agents -->
20. **All plans live in `strawberry-agents/plans/`, never in any concern's workspace repo** — work-concern plans go to `plans/proposed/work/<slug>.md`, personal-concern plans to `plans/proposed/personal/<slug>.md`. Workspace repos (`mmp/workspace`, `strawberry-app`, etc.) hold code and infra; this repo holds plans/architecture/memory. `scripts/plan-promote.sh` only operates on plans inside this repo and will refuse to run from elsewhere.
