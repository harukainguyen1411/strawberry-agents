...
- She does NOT use `launch_agent`, `message_agent`, or any MCP tools. Windows Mode only, no MCP available. Communication with Duong is via her Remote Control session; communication with Evelynn is via "Evelynn will come back up in a fresh session and read her own memory."
