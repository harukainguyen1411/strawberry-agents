...
- **Agent teams feature:** `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` is enabled. `TeamCreate`/`SendMessage`/shared TaskList workflow is Evelynn's **primary** multi-agent coordination surface. Use TeamCreate for ANY work that may iterate — not only "when Duong says have a team". Agents in a team share a task list and can communicate via SendMessage. See `runbooks/agent-team-mode.md` §Policy for the full mandate.
...
- **Agent self-close rule (2026-04-12):** All agents (except Yuumi and Skarner) run `/end-subagent-session <name>` as their FINAL action automatically — no instruction from Evelynn needed. The rule in all `.claude/agents/*.md` files was updated. Do NOT include session-close instructions in delegation prompts anymore. Teammates (any agent dispatched with `team_name` + `name`) follow the teammate lifecycle in `_shared/teammate-lifecycle.md` instead — they self-close ONLY on `shutdown_request` after emitting `shutdown_ack`.
...
- **One true gate confirmed** — Pre-commit plan-promote-guard is dead. CLAUDE.md Rule 19 + `architecture/plan-lifecycle.md` (legacy path; current canonical: architecture/agent-network-v1/plan-lifecycle.md) cleaned of stale references (`3e0c3d9` + `f3dd1f4`). PreToolUse is the sole enforcement layer.
...
- Remove from Infrastructure / Open Threads: auto-rebase workflow (deleted PR #51). Stale-branch pattern now documented in `architecture/git-workflow.md` (legacy path; current canonical: architecture/agent-network-v1/git-workflow.md) as on-demand `gh pr update-branch`.
