...
- 2026-04-20: Orianna-gated plan lifecycle ADR → `plans/proposed/2026-04-20-orianna-gated-plan-lifecycle.md` (commit 1bb8620). Signature mechanism: frontmatter `orianna_signature_<phase>` with body-hash, tamper-evidence via git commit author identity (proposed `orianna@agents.strawberry.local`). Phase checks: approved (existing fact-check + gating-Q scan), in-progress (task list + AI-minute estimates + Caitlyn section), implemented (grep re-verify + architecture_changes OR explicit architecture_impact: none). Escape hatch: `Orianna-Bypass:` trailer, Duong-only. 8 gating questions for Duong, biggest: Q1 Orianna git identity, Q2 how to distinguish Duong-human from agent commits for bypass.
