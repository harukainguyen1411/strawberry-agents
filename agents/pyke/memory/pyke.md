...
- 2026-04-09 (subagent, delivery-pipeline team, opus-4.6[1m]): Wrote `assessments/2026-04-09-delivery-pipeline-security.md` for auto-approve + auto-deploy pipeline. 5 must-haves: M1 author allowlist (Kat), M2 server-side scope check as separate job (Fiora), M3 enable Dependabot/secret-scanning/push-protection (Fiora — currently disabled), M4 real branch protection + CODEOWNERS (Fiora — currently toothless: no required reviews/checks/restrictions), M5 rotate GITHUB_TRIAGE_PAT to `repo`-only (drop `workflow` scope). Biggest hidden risks: self-hosted Hetzner runner running attacker-controlled Claude code; classic PAT with `workflow` scope can rewrite pipeline.yml itself; `sanitizeInput()` is markdown-stripper not prompt-injection defense. Dispatched coordination to Fiora + Katarina, flagged M5 to team-lead for Duong.
