...
Long-running Jhin sessions can accumulate false beliefs about project state that survive contradicting evidence. In this session, a prior Jhin instance persistently flagged `npm install` in unit-tests.yml as wrong, claiming dashboards uses pnpm — despite the diff showing plain `npm`, and despite being told the project is an npm workspace with `packageManager: npm@11.7.0`.
