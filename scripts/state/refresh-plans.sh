...
while IFS= read -r -d '' md_file; do
    ...
    db_write_tx "$DB_PATH" \
        "INSERT INTO plans_index (path,status,concern,owner,project,created,refreshed_at)
         VALUES ('$(_esc "$rel_path")','$(_esc "$status")','$(_esc "${concern:-personal}")',
                 '$(_esc "$owner")','$(_esc "$project")','$(_esc "$created")',
                 strftime('%Y-%m-%d %H:%M:%f','now'))
         ON CONFLICT(path) DO UPDATE SET
           status=excluded.status, concern=excluded.concern,
           owner=excluded.owner, project=excluded.project,
           created=excluded.created, refreshed_at=strftime('%Y-%m-%d %H:%M:%f','now');"
    rows_out=$((rows_out + 1))
