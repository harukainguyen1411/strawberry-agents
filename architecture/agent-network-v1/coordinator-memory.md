...
| 5 | TL;DR parsing produces "(no summary extractable)" | Medium | Low — index row less useful but shard still findable by UUID | Shard header convention documented in `architecture/agent-network-v1/coordinator-memory.md` §4.4 of the plan; fallback to prose parsing. |
